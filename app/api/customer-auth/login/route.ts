...
import { clearRateLimit, consumeRateLimit, isSameOriginMutation, rateLimitIdentifier, requestIp } from "@/lib/server/request-security";

const schema = z.object({ phone: z.string().trim().min(8), password: z.string().min(6) });
...
export async function POST(request: Request) {
  ...
  const identity = rateLimitIdentifier(request, phone);
  const [ipLimit, accountLimit] = await Promise.all([
    ...
    consumeRateLimit({ scope: "customer-login-account", identifier: identity, limit: 8, windowMs: 15 * 60_000, blockMs: 30 * 60_000 }),
  ]);
  ...
  const passwordMatches = verifyPassword(parsed.data.password, account?.passwordHash ?? DUMMY_PASSWORD_HASH);
  if (!account || !passwordMatches) {
    return NextResponse.json({ error: "Số điện thoại hoặc mật khẩu chưa đúng." }, { status: 401 });
  }
