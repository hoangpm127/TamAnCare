...
export async function GET(request: Request, { params }: { params: Promise<{ assetId: string }> }) {
  ...
  const buffer = await QRCode.toBuffer(`${origin}/xg-ref/${encodeURIComponent(asset.code)}`, { width: 720, margin: 2, errorCorrectionLevel: "H", color: { dark: "#4c191b", light: "#ffffff" } });
