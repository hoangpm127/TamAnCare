...
export default async function TherapistIncomePage() {
  ...
  return <div className="mx-auto max-w-4xl px-3 py-4 sm:px-6">
    <header className="overflow-hidden rounded-3xl bg-gradient-to-br from-[#291714] via-[#63301f] to-[#c64b32] p-5 text-center text-white shadow-xl">
      <p className="flex items-center justify-center gap-1.5 text-[10px] font-bold uppercase tracking-[0.15em] text-[#e7c878]"><CircleDollarSign size={15} /> Thu nhập cá nhân</p>
    ...
    <section className="mt-3 rounded-2xl border border-[#d2ad5d]/55 bg-white p-3.5 shadow-sm">
      <div className="flex items-center justify-between gap-3"><div><h2 className="flex items-center gap-1.5 text-sm font-semibold"><HandCoins size={16} className="text-[#76551d]" /> Lịch sử Tip KTV</h2><p className="mt-0.5 text-[9px] text-[#826f66]">Theo khách, dịch vụ và trạng thái chi trả</p></div><WalletCards size={19} className="text-[#c64b32]" /></div>
      <div className="mt-3 divide-y divide-[#f0e6df]">{tips.map((tip) => <article key={tip.id} className="flex items-center justify-between gap-3 py-3"><div className="min-w-0"><p className="truncate text-[11px] font-semibold">{tip.booking?.customer.fullName ?? tip.officeEvent?.companyName ?? "Tâm An Center"}</p><p className="mt-0.5 truncate text-[9px] text-[#826f66]">{tip.booking?.service.name ?? "Tâm An Business"} · {tip.serviceDate.toLocaleDateString("vi-VN", { timeZone: "Asia/Ho_Chi_Minh" })}</p></div><div className="shrink-0 text-right"><strong className="text-xs text-[#76551d]">{formatMoney(tip.amount)}</strong><p className={tip.status === "PAID" ? "text-[8px] font-semibold text-[#0b5d45]" : "text-[8px] font-semibold text-[#c64b32]"}>{tip.status === "PAID" ? "Đã chi cuối ngày" : tip.status === "PENDING" ? "Chờ chi cuối ngày" : "Đã hủy"}</p></div></article>)}{tips.length === 0 ? <p className="py-8 text-center text-xs text-[#826f66]">Chưa có khoản Tip nào được ghi nhận.</p> : null}</div>
    </section>
  </div>;
}
