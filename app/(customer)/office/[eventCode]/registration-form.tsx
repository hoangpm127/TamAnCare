...
export function OfficeRegistrationForm({ event }: { event: OfficeEventRegistrationView }) {
  ...
  return (
    <main className="bg-[#fffaf6] px-4 py-6 text-[#191414] sm:px-6">
        ...
        {done ? (
          ...
          <div className="mt-5 space-y-4">
            <label className="block">
              ...
              <input value={fullName} onChange={(target) => setFullName(target.currentTarget.value)} className="mt-2 w-full rounded-xl border border-[#eadbd1] px-3 py-3" />
            </label>
            <label className="block">
              <span className="text-sm font-semibold">Số điện thoại</span>
              <input value={phone} onChange={(target) => setPhone(target.currentTarget.value)} className="mt-2 w-full rounded-xl border border-[#eadbd1] px-3 py-3" />
            </label>
            <div>
