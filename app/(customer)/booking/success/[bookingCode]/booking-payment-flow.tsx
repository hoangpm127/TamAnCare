    ...
    return (
      <main className="flex min-h-[calc(100vh-56px)] items-center justify-center bg-[#fffaf6] px-4 py-8 text-[#191414]">
        <section className="w-full max-w-md rounded-2xl border border-[#eadbd1] bg-white p-6 text-center shadow-sm">
          <Clock3 className="mx-auto text-[#9f1d20]" size={30} />
        ...
        <section className="w-full max-w-md overflow-hidden rounded-2xl border border-[#eadbd1] bg-white shadow-lg">
          ...
          <div className="space-y-2 px-5 py-4 text-sm text-[#4d403a]">
            <p className="flex items-center gap-2">
              <CalendarClock size={15} className="shrink-0 text-[#9f1d20]" />
              {format(appointmentTime, "HH:mm")} · {format(appointmentTime, "dd/MM/yyyy")} · {summary.durationMin} phút
            </p>
            <p className="flex items-center gap-2">
              <UserRound size={15} className="shrink-0 text-[#9f1d20]" />
              {summary.therapistLabel}{summary.nickName ? ` · Khách ${summary.nickName}` : ""}
            </p>
            <p className="flex items-center gap-2">
              <MapPin size={15} className="shrink-0 text-[#9f1d20]" /> {branchInfo.label} · {branchInfo.address}
            </p>
          </div>
          ...
          <div className="relative border-t border-dashed border-[#eadbd1] px-5 py-5 text-center">
            ...
            {adminConfirmed ? (
              <Link href={`/check-in?bookingCode=${draft.referenceCode}`} className="mt-3 inline-flex items-center gap-1.5 rounded-full bg-[#9f1d20] px-4 py-2.5 text-xs font-semibold text-white">
          ...
          <div className="border-t border-dashed border-[#eadbd1] px-5 py-4">
            <p className="mb-2.5 flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.12em] text-[#9f1d20]">
            ...
            <div className="mt-3 space-y-2 border-t border-dashed border-[#eadbd1] pt-3 text-sm">
              ...
              {summary.packageName ? (
                ...
                <>
                  ...
                  <div className="flex items-center justify-between rounded-xl bg-[#fff2ef] px-3 py-2.5 text-[#9f1d20]">
          ...
          <div className="flex gap-2 border-t border-dashed border-[#eadbd1] p-4">
            <Link href="/don-cua-toi?tab=upcoming" className="flex-1 rounded-full bg-[#9f1d20] px-4 py-2.5 text-center text-sm font-semibold text-white">Lịch đã đặt</Link>
            <Link href="/vi" className="flex-1 rounded-full border border-[#9f1d20] px-4 py-2.5 text-center text-sm font-semibold text-[#9f1d20]">Xem Thu - Chi</Link>
          </div>
        </section>
      </main>
    );
    ...
    <main className="mx-auto max-w-3xl bg-[#fffaf6] px-4 py-6 text-[#191414] sm:px-6">
      <section className="overflow-hidden rounded-2xl border border-[#eadbd1] bg-white shadow-lg">
        ...
        <div className="border-b border-dashed border-[#eadbd1] px-4 py-3 text-sm text-[#4d403a]">
          <div className="flex items-center justify-between gap-2"><p className="font-semibold text-[#191414]">Dịch vụ đã chọn</p><span className="rounded-full bg-[#fff2ef] px-2 py-1 text-[9px] font-semibold text-[#9f1d20]">{summary.count} khách</span></div>
          <div className="mt-2 grid gap-1.5 sm:grid-cols-2">
            {summary.items.map((item) => <div key={item.name} className="flex items-center justify-between gap-2 rounded-lg bg-[#fffaf6] px-2.5 py-2"><span className="min-w-0 truncate text-[11px] font-medium"><strong className="mr-1 text-[#9f1d20]">{item.qty}×</strong>{item.name}</span><span className="shrink-0 text-[10px] font-semibold">{formatMoney(item.amount)}</span></div>)}
          </div>
          <div className="mt-2 flex flex-wrap gap-x-3 gap-y-1 text-[10px] text-[#665b55]">
          <p className="flex items-center gap-2"><CalendarClock size={15} className="text-[#9f1d20]" /> {format(appointmentTime, "HH:mm dd/MM/yyyy")} · {summary.durationMin} phút</p>
          <p className="flex items-center gap-2"><MapPin size={15} className="text-[#9f1d20]" /> {branchInfo.label}</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-3 divide-x divide-[#eadbd1] border-b border-dashed border-[#eadbd1] px-3 py-4 text-center">
          ...
          <div className="px-2"><p className="text-[10px] uppercase text-[#8a7a72]">{summary.packageName ? "Lượt gói" : "Cọc ngay"}</p><p className="mt-1 text-sm font-bold text-[#9f1d20]">{summary.packageName ? summary.count : formatMoney(summary.depositAmount)}</p></div>
        ...
        <div className="p-5">
          {stage === "awaiting" ? (
            ...
            <div className="flex items-center justify-center gap-2 rounded-2xl bg-[#fff2ef] p-5 text-sm font-semibold text-[#9f1d20]">
            ...
            <div className="rounded-2xl border border-[#e8d39e] bg-gradient-to-br from-[#fffaf0] to-white p-4 text-center shadow-sm">
              <span className="mx-auto flex h-11 w-11 items-center justify-center rounded-full bg-[#fff1c7] text-[#9f1d20]"><Clock3 size={23} /></span>
              ...
              <div className="mt-3 grid grid-cols-2 gap-2 rounded-xl bg-white p-3 text-xs">
                <span className="text-[#8a7a72]">Số tiền<strong className="mt-1 block text-sm text-[#9f1d20]">{formatMoney(summary.depositAmount)}</strong></span>
              ...
              <div className="mt-3 grid grid-cols-2 gap-2"><button type="button" onClick={() => void checkPaymentNow()} disabled={checkingStatus} className="flex items-center justify-center gap-1.5 rounded-full border border-[#9f1d20] px-3 py-2.5 text-xs font-semibold text-[#9f1d20] disabled:opacity-55">{checkingStatus ? <Loader2 className="animate-spin" size={13} /> : null} Kiểm tra ngay</button><Link href="/don-cua-toi?tab=upcoming" className="rounded-full bg-[#9f1d20] px-3 py-2.5 text-xs font-semibold text-white">Về lịch của tôi</Link></div>
              <button type="button" onClick={() => setStage("banking")} className="mt-3 text-xs font-semibold text-[#9f1d20] underline underline-offset-4">Xem lại thông tin chuyển khoản</button>
            </div>
          ) : (
            <>
              <p className="mb-2.5 flex items-center gap-2 text-sm font-semibold"><Wallet size={16} className="text-[#9f1d20]" /> Quét VietQR hoặc mở ứng dụng ngân hàng</p>
