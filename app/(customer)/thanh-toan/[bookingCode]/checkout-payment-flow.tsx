...
export function CheckoutPaymentFlow({ bookingCode }: { bookingCode: string }) {
  ...
  if (loading) return <main className="flex min-h-[65vh] items-center justify-center bg-[#fffaf6] text-sm text-[#665b55]"><Loader2 className="mr-2 animate-spin text-[#d13f1f]" size={18} /> Đang tải Bill cần thanh toán...</main>;
  if (!booking) return <main className="mx-auto max-w-md px-4 py-10 text-center"><section className="rounded-2xl border border-[#eadbd1] bg-white p-6"><ReceiptText className="mx-auto text-[#d13f1f]" /><h1 className="mt-3 text-lg font-semibold">Chưa thể mở thanh toán</h1><p className="mt-2 text-sm leading-6 text-[#665b55]">{error || "Không tìm thấy Bill."}</p><Link href="/don-cua-toi" className="mt-4 inline-flex rounded-full bg-[#d13f1f] px-5 py-2.5 text-sm font-semibold text-white">Về đơn của tôi</Link></section></main>;

  const paid = booking.paymentStatus === "PAID" || booking.checkoutPayment?.status === "CONFIRMED" || booking.dueAmount === 0;
  if (paid) return <main className="mx-auto max-w-md px-4 py-8 text-center"><section className="overflow-hidden rounded-2xl border border-[#eadbd1] bg-white shadow-lg"><div className="bg-gradient-to-br from-[#7a2318] to-[#231514] p-6 text-white"><CheckCircle2 className="mx-auto text-[#f5d982]" size={36} /><h1 className="mt-3 text-xl font-semibold">Đã thanh toán</h1><p className="mt-1 text-xs text-white/70">{displayBookingCode(booking.referenceCode)}</p></div><div className="p-5"><p className="text-sm leading-6 text-[#665b55]">Quầy đã xác nhận nhận đủ phần còn lại của Bill. Doanh thu cơ sở và phí nền tảng đã được hạch toán tách biệt.</p><p className="mt-3 rounded-xl bg-[#fff7ec] p-3 text-xs leading-5 text-[#805914]">Tip hoàn toàn tùy tâm và được trao trực tiếp cho KTV, không nằm trong Bill này.</p><Link href="/don-cua-toi" className="mt-4 inline-flex rounded-full bg-[#d13f1f] px-5 py-2.5 text-sm font-semibold text-white">Theo dõi trạng thái dịch vụ</Link></div></section></main>;
  ...
  return (
    <main className="mx-auto max-w-2xl bg-[#fffaf6] px-4 py-6 text-[#191414]">
      <section className="overflow-hidden rounded-2xl border border-[#eadbd1] bg-white shadow-lg">
        ...
        <div className="grid grid-cols-3 divide-x divide-[#eadbd1] border-b border-dashed border-[#eadbd1] px-2 py-4 text-center"><span className="px-1 text-[10px] text-[#8a7a72]">Tổng Bill<strong className="mt-1 block text-sm text-[#191414]">{formatMoney(booking.totalAmount)}</strong></span><span className="px-1 text-[10px] text-[#8a7a72]">Đã đặt cọc<strong className="mt-1 block text-sm text-[#b42f20]">{formatMoney(booking.paidAmount)}</strong></span><span className="px-1 text-[10px] text-[#8a7a72]">Còn lại<strong className="mt-1 block text-sm text-[#d13f1f]">{formatMoney(booking.dueAmount)}</strong></span></div>
