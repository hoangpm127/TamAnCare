        ...
        {filtered.map((item) => {
          ...
          return (
            <article key={item.id} className={cn("flex gap-3 border-b border-[#f1e5dd] px-4 py-3 last:border-b-0", !item.read && "bg-gradient-to-r from-[#fff3ee] to-white")}>
              <span className={cn("flex h-10 w-10 shrink-0 items-center justify-center rounded-xl", item.type === "INVITE" ? "bg-[#fff7df] text-[#805914]" : item.type === "PROMO" ? "bg-[#edf9f2] text-[#16784a]" : "bg-[#fff2ef] text-[#d13f1f]")}><Icon size={17} /></span>
