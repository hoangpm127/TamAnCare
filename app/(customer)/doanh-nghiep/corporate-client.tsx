...
export function CorporateClient() {
      ...
      <main className="min-h-screen bg-[radial-gradient(circle_at_top,#ecd2b4_0,#fffaf6_42%,#f7ede5_100%)] px-4 py-8 text-[#191414] sm:px-6">
        <section className="mx-auto max-w-xl overflow-hidden rounded-3xl border border-[#eadbd1] bg-white shadow-lg">
          ...
          <div className="px-6 py-6">
            ...
            <Link href={`/doanh-nghiep/${persistedEventCode || inquiryCode}`} className="mt-5 inline-flex w-full items-center justify-center gap-2 rounded-full bg-[#1d6c40] px-4 py-3 text-sm font-semibold text-white">
            ...
            <div className="mt-5 space-y-2 rounded-2xl bg-[#fff7ec] p-4 text-sm text-[#4d403a]">
              ...
              <div className="flex items-center justify-between text-xs text-[#8a7a72]">
                ...
                <span className="font-semibold text-[#4d403a]">{formatMoney(amountDueOnsite)}</span>
              </div>
            </div>
      ...
      <main className="min-h-screen bg-[radial-gradient(circle_at_top,#ecd2b4_0,#fffaf6_42%,#f7ede5_100%)] px-4 py-8 text-[#191414] sm:px-6">
        <section className="mx-auto max-w-xl space-y-4">
          <div className="rounded-2xl border border-[#eadbd1] bg-white p-5 shadow-sm">
            ...
            <div className="mt-3.5 space-y-1.5 border-t border-dashed border-[#eadbd1] pt-3.5 text-sm">
              <div className="flex items-center justify-between">
                <span className="text-[#665b55]">
                  {headcount} người × {formatMoney(trial.pricePerPerson)}
                </span>
                <span className="font-semibold">{formatMoney(subtotal)}</span>
              </div>
              {discount > 0 ? (
                <div className="flex items-center justify-between text-[#1d6c40]">
                  <span>
              ...
              <div className="flex items-center justify-between text-[#665b55]">
          ...
          <div className="overflow-hidden rounded-2xl border border-[#d8b46a] bg-gradient-to-br from-[#2a1916] via-[#4b2619] to-[#7d211f] p-5 text-white shadow-xl">
            <p className="flex items-center gap-2 text-sm font-semibold text-[#f5d982]"><Wallet size={16} /> Giữ lịch bằng khoản cọc {formatMoney(depositAmount)}</p>
            ...
            <button type="button" onClick={() => void prepareDeposit()} className="mt-4 inline-flex w-full items-center justify-center gap-2 rounded-full bg-[#f5d982] px-4 py-3 text-sm font-semibold text-[#3d1f12] shadow-lg">
  ...
  if (stage === "payment") {
    return (
      <main className="min-h-screen bg-[radial-gradient(circle_at_top,#f0d9bd_0,#fffaf6_42%,#f7ede5_100%)] px-4 py-7 text-[#191414] sm:px-6">
        <section className="mx-auto max-w-xl overflow-hidden rounded-3xl border border-[#d8b46a]/70 bg-white shadow-2xl shadow-[#5c3a1e]/15">
          <div className="bg-gradient-to-br from-[#211514] via-[#4b2619] to-[#8f151a] px-5 py-6 text-white">
            <p className="flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-[0.16em] text-[#f5d982]"><ShieldCheck size={13} /> Thanh toán bảo mật · {inquiryCode}</p>
  ...
  return (
    <main className="min-h-screen bg-[radial-gradient(circle_at_top,#ecd2b4_0,#fffaf6_36%,#f7ede5_100%)] px-4 py-6 text-[#191414] sm:px-6">
      <div className="mx-auto max-w-3xl space-y-4">
        <div className="relative overflow-hidden rounded-3xl border border-[#d8b46a]/70 bg-gradient-to-br from-[#211514] via-[#4d271a] to-[#8f1f21] p-5 text-white shadow-2xl shadow-[#5c3a1e]/20">
          <div className="pointer-events-none absolute -right-10 -top-10 h-28 w-28 rounded-full bg-[#e3b23c]/20 blur-2xl" />
          <span className="inline-flex items-center gap-1.5 rounded-full border border-[#f5d982]/25 bg-white/10 px-3 py-1 text-xs font-semibold text-[#f5d982]">
          ...
          <div className="relative mt-4 flex items-start gap-2.5 rounded-2xl border border-white/15 bg-white/[0.08] p-3.5 backdrop-blur">
            <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-[#f5d982] text-[#3d1f12] shadow-sm">
              <Sparkles size={16} />
            </span>
        ...
        <div className="rounded-2xl border border-[#d8b46a]/55 bg-gradient-to-br from-white to-[#fff9f1] p-4 shadow-md shadow-[#5c3a1e]/5">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <label className="block">
              ...
              <textarea
                ...
                rows={2}
                className="mt-1.5 min-h-[68px] w-full resize-none rounded-lg border border-[#eadbd1] px-3 py-2.5 text-sm leading-5"
              />
            </label>
            <label className="block">
          ...
          <p className="mt-2.5 flex items-start gap-1.5 text-[11px] text-[#8a7a72]">
          ...
          <div className="mt-3 flex items-center justify-between rounded-lg border border-[#eadbd1] bg-[#fdf8f5] px-3.5 py-2.5">
            ...
            <div className="flex items-center gap-3">
              <button
                ...
                className="flex h-7 w-7 items-center justify-center rounded-full border border-[#eadbd1] bg-white"
          ...
          <div className="grid gap-2.5 sm:grid-cols-3">
            {corporateTrialPackages.map((item) => (
              ...
              >
                ...
                <span className="mt-1 block text-[11px] leading-4 text-[#8a7a72]">{item.description}</span>
              </button>
            ))}
          </div>
          <p className="mt-2 text-[11px] text-[#8a7a72]">Giá gói dùng thử lần đầu, chưa bao gồm phí di chuyển.</p>
        </div>

        <div className="overflow-hidden rounded-2xl border border-[#d8b46a]/55 bg-gradient-to-br from-white to-[#fff9f1] p-4 shadow-md shadow-[#5c3a1e]/5">
          <div className="grid grid-cols-[1fr_auto] items-start gap-3">
            <div className="min-w-0">
              <h2 className="flex items-center gap-2 text-base font-semibold tracking-tight">
                <Sparkles size={18} className="shrink-0" /> 4. Gói Doanh nghiệp
              </h2>
              <p className="mt-0.5 text-xs text-[#8a7a72]">Sức khỏe định kỳ cho cả công ty (tuỳ chọn)</p>
            </div>
            <button
              type="button"
            ...
            <div className="mt-3.5 grid gap-2.5 sm:grid-cols-3">
              {corporatePackageTiers.map((item) => (
                ...
                >
                  ...
                  <span className="mt-1.5 block text-[11px] text-[#8a7a72]">
                    {item.sessionsPerMonth} buổi/tháng · {item.minHeadcountPerSession}-{item.maxHeadcountPerSession} người/buổi
                  </span>
                  <ul className="mt-2 space-y-1 text-[11px] leading-4 text-[#4d403a]">
                    {item.perks.map((perk) => (
                      <li key={perk} className="flex items-start gap-1">
                        <Check size={11} className="mt-0.5 shrink-0 text-[#1d6c40]" /> {perk}
                      </li>
                    ))}
                  </ul>
                </button>
