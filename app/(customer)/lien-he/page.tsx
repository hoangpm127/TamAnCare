          ...
          return (
            <section key={branch.id} className="overflow-hidden rounded-2xl border border-[#eadbd1] bg-white shadow-sm">
              ...
              <div className="space-y-3 px-4 py-4 text-sm">
                <p className="flex items-start gap-3"><MapPin className="mt-0.5 shrink-0 text-[#9f1d20]" size={17} /><span>{branch.address}</span></p>
                <p className="flex items-start gap-3"><Clock className="mt-0.5 shrink-0 text-[#9f1d20]" size={17} /><span>{branch.openTime}–{branch.closeTime} hằng ngày · nhận lịch cuối lúc {branch.lastBookingTime}</span></p>
                <p className="flex items-start gap-3"><Phone className="mt-0.5 shrink-0 text-[#9f1d20]" size={17} /><span>{branch.phone || "Hotline đang cập nhật"}</span></p>
              </div>
              <div className="grid gap-2 border-t border-[#f1e5dd] p-3 sm:grid-cols-2">
                {branch.phone ? <a href={`tel:${branch.phone.replace(/\s/g, "")}`} className="inline-flex items-center justify-center gap-2 rounded-full bg-[#9f1d20] px-4 py-2.5 text-sm font-semibold text-white"><Phone size={16} /> Gọi {branch.phone}</a> : null}
