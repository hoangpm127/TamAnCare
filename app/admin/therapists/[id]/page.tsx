  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-4 sm:px-6 sm:py-6 lg:px-10">
      <section className="rounded-lg border border-[#e7d6ca] bg-white p-4 shadow-sm sm:p-5">
        <p className="text-sm font-semibold uppercase tracking-[0.18em] text-[#c64b32]">Hồ sơ KTV</p>
        <h1 className="mt-1 text-2xl font-semibold">{therapist.fullName}</h1>
        <p className="mt-1 text-xs font-semibold text-[#c64b32]">{therapist.branch.name}</p>
        <p className="mt-2 text-[#68574f]">{therapist.skills.join(", ")}</p>
      ...
      <section className="mt-4 rounded-xl border border-[#e7d6ca] bg-white p-4 shadow-sm sm:p-5">
        <div className="flex items-center justify-between gap-3">
          <div>
            <h2 className="flex items-center gap-2 text-lg font-semibold"><MessageSquareText size={18} className="text-[#c64b32]" /> Lịch sử đánh giá</h2>
            <p className="mt-1 text-xs text-[#826f66]">Lưu theo khách hàng và ngày trải nghiệm, không ghi đè đánh giá cũ.</p>
          </div>
          <span className="rounded-full bg-[#fbf2e7] px-3 py-1.5 text-xs font-bold text-[#8a5a12]">{therapist.ratingAvg.toFixed(1)} ★ · {therapist.reviews.length} đánh giá</span>
        </div>
        <div className="mt-4 grid gap-3 sm:grid-cols-2">
          {therapist.reviews.map((review) => (
            <article key={review.id} className="rounded-xl border border-[#eee0d6] bg-[#fdf8f3] p-3.5">
              <div className="flex items-start justify-between gap-3">
                <div><p className="text-sm font-semibold">{review.booking.customer.fullName}</p><div className="mt-1 flex gap-0.5">{Array.from({ length: 5 }, (_, index) => <Star key={index} size={12} className={index < review.rating ? "fill-[#9f7428] text-[#9f7428]" : "text-[#dfd2ca]"} />)}</div></div>
                <time className="text-[10px] text-[#826f66]">{review.createdAt.toLocaleDateString("vi-VN")}</time>
              </div>
              <p className="mt-2 text-xs leading-5 text-[#68574f]">“{review.comment ?? "Khách chưa để lại nhận xét."}”</p>
            </article>
          ))}
          {therapist.reviews.length === 0 ? <p className="rounded-xl border border-dashed border-[#e7d6ca] p-5 text-center text-sm text-[#826f66] sm:col-span-2">Chưa có đánh giá được lưu.</p> : null}
        </div>
      </section>
      ...
      <section className="mt-5 rounded-lg border border-[#e7d6ca] bg-white p-4 shadow-sm sm:p-5">
        ...
        <div className="mt-4 space-y-3">
          {therapistBookings.length === 0 ? (
            <div className="rounded-lg bg-[#fcf3ed] p-4 text-sm text-[#68574f]">Chưa có booking hôm nay.</div>
          ) : (
            therapistBookings.map((booking) => (
              <div key={booking.id} className="rounded-lg border border-[#f2e7df] p-4">
                ...
                <p className="text-sm text-[#68574f]">{booking.customer.fullName} · {booking.service.name}</p>
              </div>
            ))
          )}
        </div>
