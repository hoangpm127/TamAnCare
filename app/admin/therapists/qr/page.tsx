...
export default async function TherapistQrCollectionPage() {
  ...
  const cards = await Promise.all(therapists.map(async (therapist) => {
    ...
    return {
      therapist,
      dataUrl: await QRCode.toDataURL(therapistCheckinUrl(token, origin), { width: 420, margin: 1, errorCorrectionLevel: "H", color: { dark: "#4c191b", light: "#ffffff" } }),
    };
  }));

  return (
    <main className="mx-auto max-w-7xl px-4 py-5 sm:px-6 lg:px-10">
      <header className="rounded-2xl bg-gradient-to-r from-[#4c191b] to-[#a85f29] p-5 text-center text-white shadow-lg">
