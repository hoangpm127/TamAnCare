  ...
  return (
    <div className="mx-auto max-w-7xl px-3 py-3 sm:px-6 sm:py-5 lg:px-10">
      ...
      <section className="mt-3 grid grid-cols-2 gap-2.5 lg:grid-cols-4">
        {[
          { label: refunds > 0 ? "Doanh thu thuần" : "Doanh thu dịch vụ", value: collected, icon: WalletCards, tone: "text-[#0b5d45] bg-[#eff7f3]", mode: "income" as DrilldownMode },
          ...
          { label: "Lãi tạm tính", value: profit, icon: TrendingUp, tone: profit >= 0 ? "text-[#0b5d45] bg-[#eff7f3]" : "text-[#c64b32] bg-[#f8ebe5]", mode: "summary" as DrilldownMode },
      ...
      <section className="mt-3 grid gap-3 lg:grid-cols-[0.72fr_1.28fr]">
            ...
            <div className="mt-3 space-y-3">{branchReports.map((item) => (
              <div key={item.branch.id} className="rounded-xl border border-[#e7d6ca] bg-[#fdf8f3] p-3">
                ...
                <div className="mt-2 grid grid-cols-3 gap-1.5 text-[9px]">
                  <button type="button" onClick={() => setDrilldown({ branchId: item.branch.id, mode: "income" })} className="rounded-lg bg-white p-2 text-left text-[#826f66]">Doanh thu<strong className="mt-0.5 block text-[10px] text-[#0b5d45]">{formatMoney(item.collected)}</strong></button>
                ...
                <div className="mt-2 h-1.5 overflow-hidden rounded-full bg-[#eee0d6]"><div className="h-full rounded-full bg-gradient-to-r from-[#c64b32] via-[#9f7428] to-[#0b5d45]" style={{ width: `${Math.max(6, Math.round((item.collected / maxBranchValue) * 100))}%` }} /></div>
              </div>
            ))}</div>
          </div>
          <div className="rounded-xl border border-[#d2ad5d]/55 bg-white p-3 shadow-sm">
        ...
        <div className="overflow-hidden rounded-xl border border-[#d2ad5d]/55 bg-white shadow-sm">
          ...
          <div className="max-h-[620px] divide-y divide-[#eee0d6] overflow-y-auto">{filteredBills.length ? filteredBills.map((bill) => {
            ...
            return <button type="button" key={bill.code} onClick={() => setSelectedBill(bill)} className="w-full p-3 text-left transition hover:bg-[#fdf8f3]"><div className="flex items-start justify-between gap-2"><div className="min-w-0"><p className="truncate text-xs font-semibold">{bill.customer} · {displayBookingCode(bill.code)}</p><p className="mt-0.5 text-[10px] text-[#826f66]">{format(bill.date, "HH:mm dd/MM/yyyy")} · {bill.branchLabel}</p></div><ChevronRight size={16} className="shrink-0 text-[#9f7428]" /></div><p className="mt-1.5 truncate text-[11px] text-[#68574f]">{bill.service}</p><div className="mt-2 grid grid-cols-3 rounded-lg bg-[#fdf8f3] px-2 py-2 text-center text-[9px] text-[#826f66]"><span>Tổng bill<strong className="mt-0.5 block text-[11px] text-[#281b18]">{formatMoney(bill.total)}</strong></span><span>{remaining ? "Đã cọc" : "Doanh thu"}<strong className="mt-0.5 block text-[11px] text-[#0b5d45]">{formatMoney(serviceCollected)}</strong></span><span>{remaining ? "Còn lại" : "Tip ngoài bill"}<strong className={cn("mt-0.5 block text-[11px]", remaining ? "text-[#c64b32]" : "text-[#76551d]")}>{formatMoney(remaining || bill.tip)}</strong></span></div></button>;
          ...
          <section className="relative flex h-[92dvh] max-h-[760px] w-full max-w-xl flex-col overflow-hidden rounded-t-[2rem] bg-[#fdf8f3] shadow-2xl sm:rounded-[2rem]">
            ...
            <div className="min-h-0 flex-1 overflow-y-auto p-4">
              {drilldown.mode === "summary" ? <div className="space-y-3">
                <button type="button" onClick={() => setDrilldown({ ...drilldown, mode: "income" })} className="flex w-full items-center gap-3 rounded-2xl border border-[#c4ddd2] bg-[#eff7f3] p-4 text-left"><span className="flex h-11 w-11 items-center justify-center rounded-xl bg-white text-[#0b5d45]"><ArrowDownLeft size={20} /></span><span className="min-w-0 flex-1"><small className="text-[10px] font-semibold uppercase tracking-wide text-[#0b5d45]">Dòng tiền vào</small><strong className="mt-0.5 block text-base">{formatMoney(drilldownReport.collected)}</strong><span className="mt-0.5 block text-[10px] text-[#567063]">Xem từng bill dịch vụ và khoản thu khác</span></span><ChevronRight size={17} className="text-[#0b5d45]" /></button>
                ...
                <div className={cn("flex items-center gap-3 rounded-2xl border p-4", drilldownReport.profit >= 0 ? "border-[#c4ddd2] bg-[#f2fbf6]" : "border-[#efc6be] bg-[#f8ebe5]")}><span className={cn("flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-white", drilldownReport.profit >= 0 ? "text-[#0b5d45]" : "text-[#c64b32]")}><TrendingUp size={20} /></span><span className="min-w-0 flex-1"><small className="text-[10px] font-semibold uppercase tracking-wide text-[#6f625b]">Lợi nhuận tạm tính</small><strong className={cn("mt-0.5 block text-base", drilldownReport.profit >= 0 ? "text-[#0b5d45]" : "text-[#c64b32]")}>{formatMoney(drilldownReport.profit)}</strong><span className="mt-0.5 block text-[10px] text-[#756962]">Doanh thu dịch vụ sau khi trừ toàn bộ chi phí</span></span></div>
              ...
              {drilldown.mode === "income" ? <div className="space-y-2">
                <div className="mb-3 flex items-center justify-between"><div><h3 className="text-sm font-semibold">Doanh thu</h3><p className="text-[10px] text-[#826f66]">Bill dịch vụ và các khoản thu khác; không bao gồm Tip KTV.</p></div><strong className="text-sm text-[#0b5d45]">{formatMoney(drilldownReport.collected)}</strong></div>
                {drilldownBills.map((bill) => <button type="button" key={bill.code} onClick={() => { setSelectedBill(bill); setDrilldown(null); }} className="flex w-full items-start gap-3 rounded-xl border border-[#e7d6ca] bg-white p-3 text-left"><span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-[#eff7f3] text-[#0b5d45]"><ReceiptText size={16} /></span><span className="min-w-0 flex-1"><strong className="block truncate text-[11px]">{bill.customer} · {bill.service}</strong><small className="mt-0.5 block text-[9px] text-[#826f66]">{displayBookingCode(bill.code)} · {format(bill.date, "HH:mm dd/MM")} · {bill.branchLabel}</small>{bill.tip ? <small className="mt-1 block text-[9px] font-semibold text-[#76551d]">Tip KTV ngoài bill: {formatMoney(bill.tip)}</small> : null}</span><strong className="shrink-0 text-[11px] text-[#0b5d45]">+{formatMoney(Math.min(bill.total, bill.collected))}</strong></button>)}
                {aggregateIncomeAmount > 0 ? <div className="flex items-start gap-3 rounded-xl border border-dashed border-[#a9d6bb] bg-[#f3fbf6] p-3"><span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-white text-[#0b5d45]"><WalletCards size={16} /></span><span className="min-w-0 flex-1"><strong className="block text-[11px]">Doanh thu tổng hợp còn lại trong kỳ</strong><small className="mt-0.5 block text-[9px] leading-4 text-[#708078]">Đã đối soát từ sổ cái; các bill chi tiết hiện có được liệt kê phía trên.</small></span><strong className="shrink-0 text-[11px] text-[#0b5d45]">+{formatMoney(aggregateIncomeAmount)}</strong></div> : null}
              </div> : null}
