...
export function AdminFinanceCenter() {
  ...
  return (
    <div className="mx-auto w-full min-w-0 max-w-7xl overflow-x-clip px-3 py-3 sm:px-6 sm:py-5 lg:px-10">
      ...
      <section className="mt-3 grid min-w-0 grid-cols-2 gap-2 sm:gap-2.5 lg:grid-cols-4">
        ...
        ].map((item) => { const Icon = item.icon; return <button type="button" onClick={() => setDrilldown({ branchId: effectiveBranch, mode: item.mode })} key={item.label} className="flex min-h-[76px] min-w-0 items-center gap-2 rounded-xl border border-[#d2ad5d]/60 bg-white p-2 text-left shadow-sm transition active:scale-[0.98] sm:gap-2.5 sm:p-2.5"><span className={cn("flex h-8 w-8 shrink-0 items-center justify-center rounded-xl sm:h-9 sm:w-9", item.tone)}><Icon size={16} /></span><span className="min-w-0 flex-1"><span className="block text-[10px] leading-3 text-[#826f66]">{item.label}</span><strong className="mt-0.5 block overflow-hidden text-ellipsis whitespace-nowrap text-xs sm:text-sm">{formatMoney(item.value)}</strong><small className="mt-0.5 flex items-center gap-1 whitespace-nowrap text-[9px] font-medium text-[#c64b32]"><Eye size={10} /> Xem chi tiết</small></span></button>; })}
      </section>
      ...
      <section className="mt-3 grid min-w-0 gap-3 lg:grid-cols-[minmax(0,0.72fr)_minmax(0,1.28fr)]">
          ...
          <div className="rounded-xl border border-[#d2ad5d]/55 bg-white p-3 shadow-sm">
            ...
            <div className="mt-3 space-y-3">{branchReports.map((item) => (
              <div key={item.branch.id} className="rounded-xl border border-[#e7d6ca] bg-[#fdf8f3] p-3">
                ...
                <div className="mt-2 grid min-w-0 grid-cols-3 gap-1.5 text-[9px]">
                  <button type="button" onClick={() => setDrilldown({ branchId: item.branch.id, mode: "income" })} className="min-w-0 rounded-lg bg-white p-2 text-left text-[#826f66]">Doanh thu<strong className="mt-0.5 block overflow-hidden text-ellipsis whitespace-nowrap text-[10px] text-[#76551d]">{formatMoney(item.collected)}</strong></button>
                  <button type="button" onClick={() => setDrilldown({ branchId: item.branch.id, mode: "expense" })} className="min-w-0 rounded-lg bg-white p-2 text-left text-[#826f66]">Chi phí<strong className="mt-0.5 block overflow-hidden text-ellipsis whitespace-nowrap text-[10px] text-[#c64b32]">{formatMoney(item.expenses)}</strong></button>
                  <button type="button" onClick={() => setDrilldown({ branchId: item.branch.id, mode: "tip" })} className="min-w-0 rounded-lg bg-white p-2 text-left text-[#826f66]">Tip KTV<strong className="mt-0.5 block overflow-hidden text-ellipsis whitespace-nowrap text-[10px] text-[#76551d]">{formatMoney(item.tips)}</strong></button>
                </div>
        ...
        <div className="min-w-0 overflow-hidden rounded-xl border border-[#d2ad5d]/55 bg-white shadow-sm">
          ...
          <div className="max-h-[620px] divide-y divide-[#eee0d6] overflow-y-auto">{filteredBills.length ? filteredBills.map((bill) => {
            ...
            return <button type="button" key={bill.code} onClick={() => setSelectedBill(bill)} className="w-full min-w-0 p-3 text-left transition hover:bg-[#fdf8f3]"><div className="flex items-start justify-between gap-2"><div className="min-w-0"><p className="truncate text-xs font-semibold">{bill.customer} · {displayBookingCode(bill.code)}</p><p className="mt-0.5 truncate text-[10px] text-[#826f66]">{format(bill.date, "HH:mm dd/MM/yyyy")} · {bill.branchLabel}</p></div><ChevronRight size={16} className="shrink-0 text-[#9f7428]" /></div><p className="mt-1.5 truncate text-[11px] text-[#68574f]">{bill.service}</p><div className="mt-2 grid min-w-0 grid-cols-3 rounded-lg bg-[#fdf8f3] px-1.5 py-2 text-center text-[9px] text-[#826f66] sm:px-2"><span className="min-w-0">Tổng bill<strong className="mt-0.5 block overflow-hidden text-ellipsis whitespace-nowrap text-[10px] text-[#281b18] sm:text-[11px]">{formatMoney(bill.total)}</strong></span><span className="min-w-0">{remaining ? "Đã cọc" : "Doanh thu"}<strong className="mt-0.5 block overflow-hidden text-ellipsis whitespace-nowrap text-[10px] text-[#76551d] sm:text-[11px]">{formatMoney(serviceCollected)}</strong></span><span className="min-w-0">{remaining ? "Còn lại" : "Tip ngoài bill"}<strong className={cn("mt-0.5 block overflow-hidden text-ellipsis whitespace-nowrap text-[10px] sm:text-[11px]", remaining ? "text-[#c64b32]" : "text-[#76551d]")}>{formatMoney(remaining || bill.tip)}</strong></span></div></button>;
