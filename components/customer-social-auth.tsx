      ...
      {providers.filter((provider) => availableProviders.includes(provider.slug as "google" | "facebook") || (linkMode && linkedProviders.includes(provider.id))).map((provider) => {
        ...
        return linked ? (
          ...
          <a
            ...
            className="flex items-center justify-center gap-2 rounded-full border border-[#ddcfc6] bg-white px-4 py-2.5 text-sm font-semibold text-[#3f3733] transition hover:border-[#c64b32] hover:bg-[#fcf5ef]"
          >
    ...
    return (
      <main className="mx-auto max-w-xl px-4 py-6 text-[#281b18] sm:px-6">
        <section className="rounded-3xl bg-white p-6 text-center shadow-xl ring-1 ring-[#e7d6ca]">
          <ShieldCheck className="mx-auto text-[#c64b32]" size={32} />
          ...
          <a href={`/api/customer-auth/oauth/${provider}?returnTo=${encodeURIComponent(returnTo)}`} className="mt-4 block rounded-full bg-[#c64b32] px-5 py-2.5 text-sm font-semibold text-white">Đăng nhập lại bằng {providerLabel}</a>
      ...
      <section className="overflow-hidden rounded-3xl bg-white shadow-xl ring-1 ring-[#e7d6ca]">
        <div className="bg-gradient-to-br from-[#2b1815] via-[#5c2718] to-[#93352d] px-5 py-6 text-white">
          <div className="flex items-start justify-between gap-4">
            <div>
              <p className="text-xs font-bold uppercase tracking-[0.15em] text-[#e7c878]">Đã xác thực qua {providerLabel}</p>
        ...
        <form onSubmit={submit} className="space-y-3 p-5">
          ...
          <div className="space-y-2 rounded-2xl bg-[#fcf5ef] p-3 ring-1 ring-[#e7d6ca]">
            <label className="flex cursor-pointer items-start gap-2.5 text-xs leading-5 text-[#554842]">
              ...
              <span>Tôi đã đọc và đồng ý với <Link href="/dieu-khoan" target="_blank" className="font-semibold text-[#c64b32] underline">Điều khoản sử dụng</Link> và <Link href="/chinh-sach-rieng-tu" target="_blank" className="font-semibold text-[#c64b32] underline">Chính sách bảo vệ dữ liệu</Link>.</span>
            </label>
            <label className="flex cursor-pointer items-start gap-2.5 text-xs leading-5 text-[#6f625c]">
              <input type="checkbox" checked={marketingOptIn} onChange={(event) => setMarketingOptIn(event.target.checked)} className="mt-1 h-4 w-4 shrink-0 accent-[#c64b32]" />
