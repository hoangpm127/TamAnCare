...
export function BusinessReviewForm({ eventCode, initialRating, initialComment }: { eventCode: string; initialRating?: number | null; initialComment?: string | null }) {
  ...
  return <section className="rounded-[2rem] border border-[#d2ad5d] bg-gradient-to-br from-[#fffaf0] to-white p-5 shadow-lg"><p className="text-xs font-bold uppercase tracking-[0.14em] text-[#c64b32]">Chất lượng phục vụ</p><h2 className="mt-1 text-xl font-semibold">Đánh giá đoàn Business</h2><div className="mt-4 flex justify-center gap-2">{[1,2,3,4,5].map((value) => <button type="button" key={value} onClick={() => { setRating(value); setSaved(false); }} aria-label={`${value} sao`} className="p-1"><Star size={30} className={value <= rating ? "fill-[#c59a3d] text-[#c59a3d]" : "text-[#d8cec8]"} /></button>)}</div><textarea value={comment} onChange={(event) => { setComment(event.target.value); setSaved(false); }} rows={3} maxLength={1000} placeholder="Điều bạn hài lòng hoặc cần Tâm An cải thiện..." className="mt-4 w-full rounded-2xl border border-[#e7d6ca] bg-white px-4 py-3 text-sm outline-none focus:border-[#c64b32]"/><button type="button" onClick={() => void submit()} disabled={busy} className="mt-3 inline-flex w-full items-center justify-center gap-2 rounded-full bg-[#c64b32] px-4 py-3 text-sm font-semibold text-white disabled:opacity-60">{busy ? <Loader2 size={16} className="animate-spin" /> : saved ? <CheckCircle2 size={16} /> : null}{saved ? "Đã ghi nhận đánh giá" : "Gửi đánh giá"}</button>{error ? <p className="mt-2 text-center text-xs text-red-700">{error}</p> : null}</section>;
}
