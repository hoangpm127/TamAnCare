        ...
        <div className="fixed inset-0 z-[90] bg-black/45 backdrop-blur-[2px]" role="presentation" onClick={() => setOpen(false)}>
          <section role="dialog" aria-modal="true" aria-label="Chọn ứng dụng ngân hàng" onClick={(event) => event.stopPropagation()} className="absolute inset-x-0 bottom-0 mx-auto flex max-h-[82dvh] w-full max-w-xl flex-col overflow-hidden rounded-t-[1.75rem] bg-[#fffaf6] shadow-2xl">
            <div className="border-b border-[#eadbd1] bg-white px-4 pb-3 pt-4">
              <div className="flex items-start justify-between gap-3"><div><p className="flex items-center gap-1.5 text-sm font-bold"><Landmark size={16} className="text-[#d13f1f]" /> Chọn ứng dụng ngân hàng</p><p className="mt-1 text-[10px] leading-4 text-[#8a7a72]">App chỉ được mở để bạn chủ động thanh toán tại quầy. Tâm An không hiển thị QR hoặc số tài khoản trên màn hình này.</p></div><button type="button" onClick={() => setOpen(false)} aria-label="Đóng" className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-[#f5efeb]"><X size={16} /></button></div>
              <label className="mt-3 flex items-center gap-2 rounded-full border border-[#e4d5cc] bg-[#fffaf6] px-3 py-2.5"><Search size={15} className="text-[#d13f1f]" /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Tìm ngân hàng..." className="min-w-0 flex-1 bg-transparent text-sm outline-none" /></label>
            </div>
            <div className="min-h-0 flex-1 overflow-y-auto px-3 py-3 pb-[max(1rem,env(safe-area-inset-bottom))]">
              ...
              <div className="space-y-2">{visibleApps.map((app) => <div key={app.appId} className="grid grid-cols-[1fr_auto] items-center gap-2 rounded-2xl border border-[#eadbd1] bg-white p-2 shadow-sm"><button type="button" onClick={() => launch(app)} className="flex min-w-0 items-center gap-3 text-left"><span className="flex h-10 w-10 shrink-0 items-center justify-center overflow-hidden rounded-xl bg-[#f7f2ee] text-[10px] font-bold text-[#d13f1f]">{app.appLogo ? <img src={app.appLogo} alt="" className="h-full w-full object-cover" /> : app.appId.toUpperCase().slice(0, 3)}</span><span className="min-w-0 flex-1"><span className="block truncate text-xs font-semibold">{app.appName.replace(/^‎/, "")}</span><span className="mt-0.5 block truncate text-[9px] text-[#8a7a72]">{app.bankName}</span></span><ChevronRight size={15} className="text-[#d13f1f]" /></button><a href={appStoreSearch(app.appName, apple)} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 rounded-full bg-[#f5efeb] px-2.5 py-2 text-[9px] font-semibold text-[#665b55]">Chưa cài? <ExternalLink size={10} /></a></div>)}</div>
