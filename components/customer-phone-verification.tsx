  ...
  return (
    <div className="rounded-2xl bg-[#fff8f3] p-3 ring-1 ring-[#eadbd1]">
      <div className="flex items-start gap-2.5">
        <MessageSquareText size={17} className="mt-0.5 shrink-0 text-[#9f1d20]" />
        ...
        <div className="mt-3 space-y-2">
          ...
          <div className="flex gap-2">
            <input
              ...
              className="min-w-0 flex-1 rounded-xl border border-[#eadbd1] bg-white px-3 py-2.5 text-center font-mono text-lg tracking-[0.25em] outline-none focus:border-[#9f1d20]"
            />
            <button type="button" disabled={busy || code.length !== config.codeLength} onClick={() => void verifyCode()} className="flex items-center justify-center gap-1.5 rounded-xl bg-[#9f1d20] px-4 text-xs font-semibold text-white disabled:opacity-50">
              {busy ? <Loader2 size={14} className="animate-spin" /> : <ShieldCheck size={14} />} Xác nhận
            </button>
          </div>
          <button type="button" disabled={busy} onClick={() => void requestCode()} className="text-[11px] font-semibold text-[#9f1d20] disabled:opacity-50">Gửi lại mã</button>
        </div>
      ) : (
