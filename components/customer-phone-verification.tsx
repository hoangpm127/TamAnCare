...
export function CustomerPhoneVerification({
  ...
  if (config === undefined) {
    return <p className="flex items-center gap-2 rounded-xl bg-[#fcf5ef] p-3 text-[11px] text-[#786a63]"><Loader2 size={13} className="animate-spin" /> Đang kiểm tra kênh xác minh...</p>;
  }
    ...
    <div className="rounded-2xl bg-[#fcf5ef] p-3 ring-1 ring-[#e7d6ca]">
      ...
      {challengeId ? (
        <div className="mt-3 space-y-2">
          <p className="text-[11px] leading-5 text-[#68574f]">{message}</p>
          <div className="flex gap-2">
            <input
              ...
              className="min-w-0 flex-1 rounded-xl border border-[#e7d6ca] bg-white px-3 py-2.5 text-center font-mono text-lg tracking-[0.25em] outline-none focus:border-[#c64b32]"
            />
            <button type="button" disabled={busy || code.length !== config.codeLength} onClick={() => void verifyCode()} className="flex items-center justify-center gap-1.5 rounded-xl bg-[#c64b32] px-4 text-xs font-semibold text-white disabled:opacity-50">
              {busy ? <Loader2 size={14} className="animate-spin" /> : <ShieldCheck size={14} />} Xác nhận
            </button>
          </div>
          <button type="button" disabled={busy} onClick={() => void requestCode()} className="text-[11px] font-semibold text-[#c64b32] disabled:opacity-50">Gửi lại mã</button>
        </div>
      ) : (
