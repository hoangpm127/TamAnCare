  ...
  return (
    <div className="fixed inset-0 z-[85] flex items-end bg-black/50 sm:items-center sm:justify-center sm:p-5">
      <form onSubmit={submit} className="max-h-[92vh] w-full overflow-y-auto rounded-t-3xl bg-white p-4 pb-[calc(env(safe-area-inset-bottom)+1rem)] shadow-2xl sm:max-w-lg sm:rounded-2xl sm:p-5">
        <div className="flex items-start justify-between gap-3"><div><p className="text-[10px] font-bold uppercase tracking-[0.14em] text-[#c64b32]">Đối soát tại quầy</p><h2 className="mt-1 text-lg font-semibold">Xác nhận đã thanh toán</h2><p className="mt-1 text-[11px] text-[#826f66]">{displayBookingCode(request.bookingCode)} · {request.customerName}</p></div><button type="button" onClick={onClose} className="flex h-9 w-9 items-center justify-center rounded-full bg-[#fbf2e7]" aria-label="Đóng"><X size={17} /></button></div>
