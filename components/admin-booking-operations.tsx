    ...
    <div className="fixed inset-0 z-[70] flex items-end bg-black/45 sm:items-center sm:justify-center sm:p-5">
      <form onSubmit={submit} className="max-h-[92vh] w-full overflow-y-auto rounded-t-3xl bg-white p-4 shadow-2xl sm:max-w-xl sm:rounded-2xl">
        <div className="flex items-start justify-between gap-3">
          <div><p className="text-[10px] font-bold uppercase tracking-[0.14em] text-[#c64b32]">Booking thủ công</p><h2 className="mt-1 text-lg font-semibold">Tạo yêu cầu đặt lịch</h2></div>
        ...
        <div className="mt-4 grid gap-3 sm:grid-cols-2">
          <label className="text-xs font-semibold">Số phiếu nội bộ<input required value={code} onChange={(event) => setCode(event.target.value)} className="mt-1.5 w-full rounded-xl border border-[#e7d6ca] px-3 py-2.5 text-sm font-normal" /></label>
          <label className="text-xs font-semibold">Khách hàng<input required value={customerName} onChange={(event) => setCustomerName(event.target.value)} className="mt-1.5 w-full rounded-xl border border-[#e7d6ca] px-3 py-2.5 text-sm font-normal" placeholder="Họ tên khách" /></label>
          <label className="text-xs font-semibold">Số điện thoại<input required value={customerPhone} onChange={(event) => setCustomerPhone(event.target.value)} className="mt-1.5 w-full rounded-xl border border-[#e7d6ca] px-3 py-2.5 text-sm font-normal" inputMode="tel" /></label>
          ...
          <label className="text-xs font-semibold">Ngày giờ<input required type="datetime-local" value={timeIso} onChange={(event) => setTimeIso(event.target.value)} className="mt-1.5 w-full rounded-xl border border-[#e7d6ca] px-3 py-2.5 text-sm font-normal" /></label>
          ...
          <label className="text-xs font-semibold">Tổng dịch vụ<input readOnly value={formatMoney(totalAmount)} className="mt-1.5 w-full rounded-xl border border-[#e7d6ca] bg-[#fdf8f3] px-3 py-2.5 text-sm font-semibold" /></label>
          <label className="text-xs font-semibold">Cọc giữ chỗ 10%<input readOnly value={formatMoney(depositAmount)} className="mt-1.5 w-full rounded-xl border border-[#e7d6ca] bg-[#fdf8f3] px-3 py-2.5 text-sm font-semibold text-[#c64b32]" /></label>
        </div>
        {error ? <p className="mt-3 rounded-xl bg-red-50 p-3 text-xs font-medium text-red-700">{error}</p> : null}
        <div className="mt-4 grid grid-cols-2 gap-2"><button type="button" onClick={onClose} className="rounded-full border border-[#e7d6ca] py-2.5 text-sm font-semibold">Hủy</button><button disabled={saving} className="rounded-full bg-[#c64b32] py-2.5 text-sm font-semibold text-white disabled:opacity-60">{saving ? "Đang tạo..." : "Tạo booking"}</button></div>
      </form>
    </div>
  );
}
...
function CustomerChat({ request, initialMessage, onClose }: { request: AdminBookingRequest; initialMessage: string; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-[80] flex flex-col bg-[#fdf8f3]">
      <header className="flex items-center gap-3 border-b border-[#e7d6ca] bg-white px-4 pb-3 pt-[calc(env(safe-area-inset-top)+0.75rem)] shadow-sm">
        <button type="button" onClick={onClose} className="flex h-9 w-9 items-center justify-center rounded-full bg-[#fbf2e7]" aria-label="Đóng chat"><X size={18} /></button>
        <span className="flex h-10 w-10 items-center justify-center rounded-full bg-[#c64b32] text-white"><UserRound size={18} /></span>
        <div className="min-w-0 flex-1"><h2 className="truncate text-sm font-semibold">{request.customerName}</h2><p className="text-[10px] text-[#826f66]">{request.customerPhone} · {displayBookingCode(request.bookingCode)}</p></div>
        <Phone size={17} className="text-[#c64b32]" />
      </header>
      <div className="flex-1 overflow-y-auto p-4">
        <div className="mx-auto max-w-2xl space-y-3">
          <div className="max-w-[82%] rounded-2xl rounded-bl-md border border-[#e7d6ca] bg-white p-3 text-sm leading-5 shadow-sm">Mình đã đặt lịch {request.serviceLabel} và mong Tâm An giữ giúp khung giờ này.</div>
          {messages.map((message, index) => <div key={`${message}-${index}`} className="ml-auto max-w-[86%] rounded-2xl rounded-br-md bg-[#c64b32] p-3 text-sm leading-5 text-white shadow-sm">{message}<p className="mt-1 text-right text-[9px] text-white/70">Đã gửi tới khách hàng</p></div>)}
          <div className="rounded-2xl border border-[#c59a3d]/55 bg-[#fff7df] p-3"><p className="text-xs font-semibold text-[#6d4912]">Gợi ý giữ thiện cảm</p><p className="mt-1 text-[11px] leading-4 text-[#715943]">Xin lỗi ngắn gọn, đưa ra 1–2 phương án thay thế và ưu tiên khung giờ gần nhất cho khách.</p></div>
        </div>
      </div>
      <div className="border-t border-[#e7d6ca] bg-white p-3 pb-[calc(env(safe-area-inset-bottom)+0.75rem)]">
        <div className="mx-auto flex max-w-2xl items-end gap-2"><textarea value={draft} onChange={(event) => setDraft(event.target.value)} rows={2} className="min-h-11 flex-1 resize-none rounded-2xl border border-[#e7d6ca] px-3 py-2.5 text-sm" placeholder="Nhập tin nhắn chăm sóc khách..." /><button type="button" onClick={sendMessage} className="flex h-11 w-11 items-center justify-center rounded-full bg-[#c64b32] text-white" aria-label="Gửi"><Send size={17} /></button></div>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="fixed inset-0 z-[85] flex items-end bg-black/50 sm:items-center sm:justify-center sm:p-5">
      <form onSubmit={submit} className="max-h-[92vh] w-full overflow-y-auto rounded-t-3xl bg-white p-4 pb-[calc(env(safe-area-inset-bottom)+1rem)] shadow-2xl sm:max-w-lg sm:rounded-2xl sm:p-5">
        <div className="flex items-start justify-between gap-3"><div><p className="text-[10px] font-bold uppercase tracking-[0.14em] text-[#c64b32]">Check-out tại quầy</p><h2 className="mt-1 text-lg font-semibold">Xác nhận thanh toán & đóng Bill</h2><p className="mt-1 text-[11px] text-[#826f66]">{displayBookingCode(request.bookingCode)} · {request.customerName}</p></div><button type="button" onClick={onClose} className="flex h-9 w-9 items-center justify-center rounded-full bg-[#fbf2e7]" aria-label="Đóng"><X size={17} /></button></div>
        <div className="mt-4 rounded-xl bg-[#fdf8f3] p-3 text-center"><span className="text-[10px] text-[#826f66]">Số tiền Bill cần ghi nhận<strong className="mt-1 block text-lg text-[#c64b32]">{formatMoney(dueAmount)}</strong></span></div>
        <label className="mt-4 block text-xs font-semibold">Tiền Bill thực nhận<input required readOnly type="number" value={actualAmount} className="mt-1.5 w-full rounded-xl border border-[#e7d6ca] bg-[#f8f4f1] px-3 py-3 text-center text-lg font-bold text-[#c64b32]" /></label>
        <div className="mt-3"><p className="text-xs font-semibold">Phương thức đã nhận</p><CompactSelect className="mt-1.5" value={method} onValueChange={(value) => setMethod(value as typeof method)} dialogTitle="Phương thức thanh toán tại quầy" triggerClassName="text-sm font-normal" options={[{ value: "BANK_TRANSFER_MANUAL", label: "Chuyển khoản ngân hàng" }, { value: "CARD_POS", label: "Thanh toán thẻ" }, { value: "CASH", label: "Tiền mặt" }]} /></div>
        {method !== "CASH" ? <label className="mt-3 block text-xs font-semibold">Mã tham chiếu ngân hàng/POS<input value={externalReference} onChange={(event) => setExternalReference(event.target.value)} className="mt-1.5 w-full rounded-xl border border-[#e7d6ca] px-3 py-2.5 text-sm font-normal" placeholder="Nhập mã trên biên lai" /></label> : null}
...
export function AdminBookingOperations() {
  ...
  async function transitionBooking(request: AdminBookingRequest, status: "CONFIRMED" | "CHECKED_IN" | "IN_SERVICE" | "COMPLETED") {
    setActionError("");
  ...
  return (
    <div className="mx-auto max-w-7xl px-3 py-3 sm:px-6 sm:py-5 lg:px-10">
      ...
      <div ref={bookingDetailRef} className="mt-3 scroll-mt-28 rounded-2xl border border-[#d2ad5d]/60 bg-[#fffaf4] px-3.5 py-3 shadow-sm"><div className="flex items-center justify-between gap-3"><div><p className="text-[9px] font-bold uppercase tracking-[0.12em] text-[#c64b32]">Booking theo ngày</p><h2 className="mt-0.5 text-sm font-semibold capitalize">{new Intl.DateTimeFormat("vi-VN", { weekday: "long", day: "2-digit", month: "2-digit", year: "numeric" }).format(selectedDay)}</h2></div><span className="rounded-full bg-[#291714] px-2.5 py-1 text-[9px] font-semibold text-white">{selectedRegular.length + selectedBusiness.length} Booking</span></div></div>
      ...
      <div className="mt-3 space-y-3">
        {selectedRegular.map((request) => {
          ...
          return (
            <article key={request.bookingCode} className="overflow-hidden rounded-2xl border border-[#d2ad5d]/65 bg-white shadow-sm">
              <div className="p-3.5">
                <div className="flex items-start justify-between gap-3"><div className="min-w-0 flex-1"><p className="text-sm font-bold tracking-tight">{displayBookingCode(request.bookingCode)}</p><div className="mt-1 flex min-w-0 items-center justify-between gap-2"><p className="flex shrink-0 items-center gap-1 text-[10px] font-semibold text-[#c64b32]"><Building2 size={11} /> {request.branchLabel}</p><p className="flex min-w-0 items-center justify-end gap-1 text-[10px] text-[#826f66]"><CalendarClock size={11} className="shrink-0" /><span className="truncate">{new Date(request.timeIso).toLocaleString("vi-VN", { weekday: "short", hour: "2-digit", minute: "2-digit", day: "2-digit", month: "2-digit", year: "numeric" })}</span></p></div></div><span className={`shrink-0 rounded-full px-2.5 py-1 text-[10px] font-semibold ${status.className}`}>{status.label}</span></div>
                <div className="mt-3 space-y-2 rounded-xl bg-[#fdf8f3] p-3">
                  <p className="flex min-w-0 items-center gap-1.5 text-sm font-semibold"><UserRound size={14} className="shrink-0 text-[#c64b32]" /><span className="truncate">{request.customerName} · <span className="font-normal text-[#68574f]">{request.customerPhone}</span></span></p>
              ...
              <div className="grid border-t border-[#e7d6ca] bg-[#fffdfb] p-2.5">
                {request.rawStatus === "PENDING" ? <div className="grid grid-cols-2 gap-2"><button type="button" onClick={() => { setRejecting(request); setReason(QUICK_REASONS[0]); }} className="inline-flex items-center justify-center gap-1.5 rounded-full bg-[#c64b32] px-3 py-2.5 text-xs font-semibold text-white"><XCircle size={14} /> Từ chối & lý do</button><button type="button" onClick={() => void confirm(request)} className="inline-flex items-center justify-center gap-1.5 rounded-full bg-[#76551d] px-3 py-2.5 text-xs font-semibold text-white"><Check size={14} /> Xác nhận lịch</button></div> : request.rawStatus === "CONFIRMED" ? <button type="button" onClick={() => void transitionBooking(request, "CHECKED_IN")} className="inline-flex items-center justify-center gap-1.5 rounded-full bg-[#c64b32] py-2.5 text-xs font-semibold text-white"><UserRound size={14} /> Khách đã đến · Check-in</button> : request.rawStatus === "CHECKED_IN" ? <button type="button" onClick={() => void transitionBooking(request, "IN_SERVICE")} className="inline-flex items-center justify-center gap-1.5 rounded-full bg-[#2452b8] py-2.5 text-xs font-semibold text-white"><CheckCircle2 size={14} /> Xác nhận Bắt đầu</button> : request.rawStatus === "IN_SERVICE" ? request.paymentStatus === "PAID" ? <button type="button" onClick={() => void transitionBooking(request, "COMPLETED")} className="inline-flex items-center justify-center gap-1.5 rounded-full bg-[#76551d] py-2.5 text-xs font-semibold text-white"><CheckCircle2 size={14} /> Xác nhận check-out & đóng Bill</button> : ["OWNER", "BRANCH_MANAGER", "RECEPTIONIST"].includes(activeSession.role) ? <button type="button" onClick={() => setCollecting(request)} className="inline-flex items-center justify-center gap-1.5 rounded-full bg-[#76551d] py-2.5 text-xs font-semibold text-white"><Banknote size={14} /> Check-out & xác nhận thanh toán</button> : <p className="flex items-center justify-center gap-1.5 rounded-xl bg-[#fff7df] px-3 py-2.5 text-xs font-semibold text-[#76551d]"><Banknote size={14} /> Chờ lễ tân đối soát đủ Bill</p> : request.rawStatus === "COMPLETED" ? <p className="flex items-center justify-center gap-1.5 py-1 text-xs font-semibold text-[#76551d]"><CheckCircle2 size={15} /> Dịch vụ và Bill đã hoàn tất</p> : <button type="button" onClick={() => setChat({ request, initialMessage: request.rejectionReason ?? "Tâm An xin phép hỗ trợ bạn đổi lịch." })} className="inline-flex items-center justify-center gap-1.5 rounded-full border border-[#c64b32] py-2.5 text-xs font-semibold text-[#c64b32]"><MessageCircleMore size={15} /> Mở trao đổi với khách</button>}
              </div>
            </article>
          );
        })}
      ...
      {selectedRegular.length === 0 && selectedBusiness.length === 0 ? <div className="mt-3 rounded-2xl border border-dashed border-[#d2ad5d] bg-white p-8 text-center text-xs text-[#826f66]">Ngày này chưa có Booking phù hợp cơ sở đang lọc.</div> : null}
      ...
      {rejecting ? <div className="fixed inset-0 z-[70] flex items-end bg-black/45 sm:items-center sm:justify-center sm:p-5"><section className="max-h-[90vh] w-full overflow-y-auto rounded-t-3xl bg-white p-4 sm:max-w-lg sm:rounded-2xl"><div className="flex items-start justify-between gap-3"><div><p className="text-[10px] font-bold uppercase tracking-[0.14em] text-[#c64b32]">Phản hồi tế nhị</p><h2 className="mt-1 text-lg font-semibold">Từ chối lịch của {rejecting.customerName}</h2><p className="mt-1 text-[11px] text-[#826f66]">Chọn nhanh một lý do rồi điều chỉnh nếu cần.</p></div><button type="button" onClick={() => setRejecting(null)} className="flex h-9 w-9 items-center justify-center rounded-full bg-[#fbf2e7]"><X size={17} /></button></div><div className="mt-4 space-y-2">{QUICK_REASONS.map((item) => <button key={item} type="button" onClick={() => setReason(item)} className={`w-full rounded-xl border p-3 text-left text-xs leading-5 ${reason === item ? "border-[#c64b32] bg-[#f8ebe5] text-[#7c171a]" : "border-[#e7d6ca]"}`}>{item}</button>)}</div><label className="mt-3 block text-xs font-semibold">Nội dung gửi khách<textarea value={reason} onChange={(event) => setReason(event.target.value)} rows={4} className="mt-1.5 w-full resize-none rounded-xl border border-[#e7d6ca] p-3 text-sm leading-5" /></label><div className="mt-4 grid grid-cols-[0.8fr_1.2fr] gap-2"><button type="button" onClick={() => setRejecting(null)} className="rounded-full border border-[#e7d6ca] py-2.5 text-xs font-semibold">Quay lại</button><button type="button" onClick={rejectAndChat} className="inline-flex items-center justify-center gap-1.5 rounded-full bg-[#c64b32] py-2.5 text-xs font-semibold text-white"><MessageCircleMore size={14} /> Gửi & mở Chat</button></div></section></div> : null}
