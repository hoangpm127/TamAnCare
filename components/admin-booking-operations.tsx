...
function statusMeta(status: AdminBookingRequest["status"], rawStatus: string) {
  ...
  if (status === "REJECTED") return { label: "Cần đổi lịch", className: "bg-[#fff1ef] text-[#9f1d20]" };
    ...
    <div className="fixed inset-0 z-[70] flex items-end bg-black/45 sm:items-center sm:justify-center sm:p-5">
      <form onSubmit={submit} className="max-h-[92vh] w-full overflow-y-auto rounded-t-3xl bg-white p-4 shadow-2xl sm:max-w-xl sm:rounded-2xl">
        <div className="flex items-start justify-between gap-3">
          <div><p className="text-[10px] font-bold uppercase tracking-[0.14em] text-[#9f1d20]">Booking thủ công</p><h2 className="mt-1 text-lg font-semibold">Tạo yêu cầu đặt lịch</h2></div>
  ...
  return (
    <div className="fixed inset-0 z-[80] flex flex-col bg-[#fffaf6]">
      <header className="flex items-center gap-3 border-b border-[#eadbd1] bg-white px-4 pb-3 pt-[calc(env(safe-area-inset-top)+0.75rem)] shadow-sm">
        ...
        <span className="flex h-10 w-10 items-center justify-center rounded-full bg-[#9f1d20] text-white"><UserRound size={18} /></span>
        ...
        <Phone size={17} className="text-[#9f1d20]" />
      </header>
      <div className="flex-1 overflow-y-auto p-4">
        <div className="mx-auto max-w-2xl space-y-3">
          ...
          {messages.map((message, index) => <div key={`${message}-${index}`} className="ml-auto max-w-[86%] rounded-2xl rounded-br-md bg-[#9f1d20] p-3 text-sm leading-5 text-white shadow-sm">{message}<p className="mt-1 text-right text-[9px] text-white/70">Đã gửi tới khách hàng</p></div>)}
  ...
  return (
    <div className="fixed inset-0 z-[85] flex items-end bg-black/50 sm:items-center sm:justify-center sm:p-5">
      <form onSubmit={submit} className="max-h-[92vh] w-full overflow-y-auto rounded-t-3xl bg-white p-4 pb-[calc(env(safe-area-inset-bottom)+1rem)] shadow-2xl sm:max-w-lg sm:rounded-2xl sm:p-5">
        <div className="flex items-start justify-between gap-3"><div><p className="text-[10px] font-bold uppercase tracking-[0.14em] text-[#9f1d20]">Đối soát tại quầy</p><h2 className="mt-1 text-lg font-semibold">Xác nhận đã thanh toán</h2><p className="mt-1 text-[11px] text-[#8a7a72]">{displayBookingCode(request.bookingCode)} · {request.customerName}</p></div><button type="button" onClick={onClose} className="flex h-9 w-9 items-center justify-center rounded-full bg-[#fff7ec]" aria-label="Đóng"><X size={17} /></button></div>
        <div className="mt-4 rounded-xl bg-[#fffaf6] p-3 text-center"><span className="text-[10px] text-[#8a7a72]">Số tiền Bill cần ghi nhận<strong className="mt-1 block text-lg text-[#9f1d20]">{formatMoney(dueAmount)}</strong></span></div>
        <label className="mt-4 block text-xs font-semibold">Tiền Bill thực nhận<input required readOnly type="number" value={actualAmount} className="mt-1.5 w-full rounded-xl border border-[#eadbd1] bg-[#f8f4f1] px-3 py-3 text-center text-lg font-bold text-[#9f1d20]" /></label>
...
export function AdminBookingOperations() {
  ...
  return (
    <div className="mx-auto max-w-7xl px-3 py-3 sm:px-6 sm:py-5 lg:px-10">
      ...
      <div ref={bookingDetailRef} className="mt-3 scroll-mt-28 rounded-2xl border border-[#d8b46a]/60 bg-[#fffaf4] px-3.5 py-3 shadow-sm"><div className="flex items-center justify-between gap-3"><div><p className="text-[9px] font-bold uppercase tracking-[0.12em] text-[#9f1d20]">Booking theo ngày</p><h2 className="mt-0.5 text-sm font-semibold capitalize">{new Intl.DateTimeFormat("vi-VN", { weekday: "long", day: "2-digit", month: "2-digit", year: "numeric" }).format(selectedDay)}</h2></div><span className="rounded-full bg-[#291714] px-2.5 py-1 text-[9px] font-semibold text-white">{selectedRegular.length + selectedBusiness.length} Booking</span></div></div>
        ...
        {selectedRegular.map((request) => {
          ...
          return (
            <article key={request.bookingCode} className="overflow-hidden rounded-2xl border border-[#d8b46a]/65 bg-white shadow-sm">
              <div className="p-3.5">
                <div className="flex items-start justify-between gap-3"><div className="min-w-0 flex-1"><p className="text-sm font-bold tracking-tight">{displayBookingCode(request.bookingCode)}</p><div className="mt-1 flex min-w-0 items-center justify-between gap-2"><p className="flex shrink-0 items-center gap-1 text-[10px] font-semibold text-[#9f1d20]"><Building2 size={11} /> {request.branchLabel}</p><p className="flex min-w-0 items-center justify-end gap-1 text-[10px] text-[#8a7a72]"><CalendarClock size={11} className="shrink-0" /><span className="truncate">{new Date(request.timeIso).toLocaleString("vi-VN", { weekday: "short", hour: "2-digit", minute: "2-digit", day: "2-digit", month: "2-digit", year: "numeric" })}</span></p></div></div><span className={`shrink-0 rounded-full px-2.5 py-1 text-[10px] font-semibold ${status.className}`}>{status.label}</span></div>
                <div className="mt-3 space-y-2 rounded-xl bg-[#fffaf6] p-3">
                  <p className="flex min-w-0 items-center gap-1.5 text-sm font-semibold"><UserRound size={14} className="shrink-0 text-[#9f1d20]" /><span className="truncate">{request.customerName} · <span className="font-normal text-[#665b55]">{request.customerPhone}</span></span></p>
                  <p className="flex min-w-0 items-center gap-1.5 text-[10px] font-medium text-[#665b55]"><CalendarClock size={13} className="shrink-0 text-[#9f1d20]" /><span className="truncate">{request.serviceLabel}</span></p>
                </div>
                {request.relationship === "BOSS" || request.relationship === "FRIEND" ? <div className={request.relationship === "BOSS" ? "mt-2 flex items-start gap-2 rounded-xl bg-[#30201c] p-2.5 text-white" : "mt-2 flex items-start gap-2 rounded-xl bg-[#fff2ef] p-2.5 text-[#6f211f]"}><Handshake size={14} className={request.relationship === "BOSS" ? "mt-0.5 shrink-0 text-[#f5d982]" : "mt-0.5 shrink-0 text-[#9f1d20]"} /><div><p className="text-[10px] font-bold uppercase tracking-wide">{request.relationship === "BOSS" ? "Khách mời sếp / đối tác" : "Khách đi cùng bạn"}</p><p className={request.relationship === "BOSS" ? "mt-1 text-[10px] leading-4 text-white/70" : "mt-1 text-[10px] leading-4 text-[#80524a]"}>{request.careNote ?? "Ưu tiên sắp xếp giường gần nhau và không gian phù hợp."}</p></div></div> : null}
