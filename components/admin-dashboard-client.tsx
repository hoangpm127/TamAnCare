  ...
  return (
    <div className="mx-auto max-w-7xl px-3 py-3 sm:px-6 sm:py-5 lg:px-10">
      <section className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-[#231514] via-[#3d1f12] to-[#5c1014] p-4 text-white shadow-lg sm:p-5">
        <div className="pointer-events-none absolute -right-16 -top-16 h-56 w-56 rounded-full bg-[#e3b23c]/15 blur-3xl" />
        <div className="relative">
          ...
          <div className="mt-4 border-t border-white/15 pt-3">
            <div className="scrollbar-hide flex gap-1.5 overflow-x-auto">
              {PERIODS.map((item) => <button key={item.value} type="button" onClick={() => setPeriod(item.value)} className={cn("shrink-0 rounded-full px-3 py-1.5 text-[10px] font-semibold transition", period === item.value ? "bg-[#f5d982] text-[#3d1f12]" : "bg-white/10 text-white/75")}>{item.label}</button>)}
            </div>
      ...
      <section className="mt-3 grid grid-cols-2 gap-2.5 lg:grid-cols-4">
        {kpis.map((item, index) => { const Icon = item.icon; return <button type="button" onClick={() => setSelectedKpi(index)} key={item.label} className="flex min-h-[78px] min-w-0 items-center gap-2.5 rounded-xl border border-[#d8b46a]/55 bg-white p-2.5 text-left shadow-sm transition active:scale-[0.98]"><span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-[#fff2ef] text-[#d13f1f]"><Icon size={16} /></span><span className="min-w-0 flex-1"><span className="block truncate text-[10px] font-medium text-[#8a7a72]">{item.label}</span><span className="mt-0.5 flex items-center gap-1 text-sm font-bold tracking-tight sm:text-base">{item.value}{item.label === "Hiệu suất KTV" ? <Star size={11} className="fill-[#b9862c] text-[#b9862c]" /> : null}</span><span className="mt-0.5 line-clamp-1 text-[9px] leading-3 text-[#8a7a72]">{item.detail}</span></span></button>; })}
      </section>

      <section className="mt-3 rounded-xl border border-[#eadbd1] bg-white p-3 shadow-sm sm:p-4">
        <div className="flex items-center justify-between gap-3"><div><h2 className="text-base font-semibold">Lịch trong phạm vi ({periodLabel.toLocaleLowerCase("vi")}, {dayPartLabel.toLocaleLowerCase("vi")})</h2><p className="mt-0.5 text-[10px] text-[#8a7a72]">{branchLabel} · thay đổi đồng bộ theo bộ lọc phía trên.</p></div><Clock3 size={20} className="shrink-0 text-[#d13f1f]" /></div>
        <div className="mt-3 space-y-2 md:hidden">{scheduleRows.slice(0, 4).map((row) => <div key={row.key} className="rounded-xl bg-[#fffaf6] p-2.5"><div className="flex items-start justify-between gap-2"><p className="text-sm font-semibold">{row.customer}</p><span className="rounded-full bg-[#fff2ef] px-2 py-1 text-[9px] font-semibold text-[#d13f1f]">{row.status}</span></div><p className="mt-1 text-xs text-[#665b55]">{row.service} · {row.therapist}</p><div className="mt-2 flex justify-between text-[10px] text-[#8a7a72]"><span>{format(row.time, "HH:mm dd/MM")} · {row.branch}</span><strong className="text-[#191414]">{formatMoney(row.amount)}</strong></div></div>)}{scheduleRows.length === 0 ? <p className="rounded-xl bg-[#fffaf6] p-5 text-center text-xs text-[#8a7a72]">Không có lịch phù hợp bộ lọc.</p> : null}</div>
        <div className="mt-3 hidden overflow-x-auto md:block"><table className="w-full min-w-[680px] text-left text-xs"><thead className="text-[#8a7a72]"><tr className="border-b border-[#eadbd1]"><th className="py-2.5">Thời gian</th><th>Khách</th><th>Dịch vụ</th><th>KTV / Cơ sở</th><th>Trạng thái</th><th className="text-right">Tổng</th></tr></thead><tbody>{scheduleRows.map((row) => <tr key={row.key} className="border-b border-[#f2e7df]"><td className="py-3 font-semibold">{format(row.time, "HH:mm dd/MM")}</td><td>{row.customer}</td><td>{row.service}</td><td>{row.therapist}<span className="block text-[10px] text-[#8a7a72]">{row.branch}</span></td><td><span className="rounded-full bg-[#fff2ef] px-2 py-1 text-[10px] font-semibold text-[#d13f1f]">{row.status}</span></td><td className="text-right font-semibold">{formatMoney(row.amount)}</td></tr>)}</tbody></table></div>
      </section>

      <section className="mt-3 rounded-xl border border-[#eadbd1] bg-white p-3 shadow-sm sm:p-4">
        <h2 className="text-sm font-semibold">Truy cập nhanh</h2>
        <div className="mt-2.5 grid grid-cols-2 gap-2.5 lg:grid-cols-4">{quickSections.map((slug) => { const meta = ADMIN_SECTION_META[slug]; const Icon = meta.icon; return <Link key={slug} href={`/admin/${slug}`} className="group flex min-h-[78px] min-w-0 items-center gap-2.5 rounded-xl border border-[#d8b46a]/55 bg-[#fffdfb] p-2.5 hover:border-[#d13f1f]"><span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-[#fff7ec] text-[#7a3e1d]"><Icon size={16} /></span><span className="min-w-0 flex-1"><span className="block truncate text-[11px] font-semibold">{meta.label}</span><span className="mt-0.5 block line-clamp-2 text-[9px] leading-3 text-[#8a7a72]">{meta.shortDescription}</span></span><ArrowRight size={13} className="shrink-0 text-[#b9862c] transition group-hover:translate-x-0.5" /></Link>; })}</div>
      </section>

      <section className="mt-3 rounded-xl border border-[#e3b23c]/45 bg-[#fff7ec] p-3"><p className="flex items-center gap-1.5 text-xs font-semibold uppercase tracking-wide text-[#7a541c]"><AlertTriangle size={14} /> Cần chú ý</p><div className="mt-2 grid gap-1 text-[11px] leading-4 text-[#715943] sm:grid-cols-3"><p>• {serverOperations?.maintenanceRooms ?? 0} phòng/ghế đang bảo trì.</p><p>• {confirmedCount} lịch đã cọc đang chờ check-in.</p><p>• {pendingAdminCount} booking mới cần phản hồi.</p></div></section>

      {selectedKpi !== null ? (() => { const item = kpis[selectedKpi]; const Icon = item.icon; return <div className="fixed inset-0 z-50 flex items-end bg-black/45 pb-[env(safe-area-inset-bottom)] sm:items-center sm:justify-center sm:p-5"><section className="max-h-[90dvh] w-full overflow-y-auto rounded-t-3xl bg-white p-4 shadow-2xl sm:max-w-md sm:rounded-2xl"><div className="flex items-start gap-3"><span className="flex h-10 w-10 items-center justify-center rounded-xl bg-[#5c3a1e] text-[#f5d982]"><Icon size={18} /></span><div className="min-w-0 flex-1"><p className="text-[10px] font-semibold uppercase tracking-[0.13em] text-[#d13f1f]">Dữ liệu cấu thành</p><h2 className="mt-0.5 text-lg font-semibold">{item.label}</h2><p className="mt-0.5 text-[11px] text-[#8a7a72]">{periodLabel} · {dayPartLabel} · {branchLabel}</p></div><button type="button" onClick={() => setSelectedKpi(null)} className="flex h-9 w-9 items-center justify-center rounded-full bg-[#fff7ec]" aria-label="Đóng"><X size={17} /></button></div><div className="mt-4 space-y-2">{item.rows.map(([label, value]) => <div key={label} className="flex items-center justify-between rounded-xl border border-[#eadbd1] px-3 py-3"><span className="flex items-center gap-1.5 text-xs text-[#665b55]"><CheckCircle2 size={13} className="text-[#b9862c]" /> {label}</span><strong className="text-sm">{value}</strong></div>)}</div><Link href={item.href} onClick={() => setSelectedKpi(null)} className="mt-4 flex items-center justify-center gap-1.5 rounded-full bg-[#d13f1f] py-2.5 text-xs font-semibold text-white">Xem dữ liệu chi tiết <ArrowRight size={13} /></Link></section></div>; })() : null}
    </div>
  );
}
