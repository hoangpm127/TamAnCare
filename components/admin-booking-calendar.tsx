...
const KIND = {
  personal: { label: "Cá nhân", dot: "bg-[#d13f1f]", text: "text-[#d13f1f]", pale: "bg-[#fff0ed]" },
  ...
  business: { label: "Business", dot: "bg-[#1d9a60]", text: "text-[#16784a]", pale: "bg-[#eaf8f0]" },
} as const;
  ...
  return (
    <div className="mt-2.5 rounded-xl bg-white/10 p-1.5 ring-1 ring-white/10">
      <div className="flex items-center justify-center gap-1.5 text-center text-[9px] font-semibold text-white/75">
        {saving || !view ? <Loader2 className="animate-spin" size={11} /> : view.mode === "AUTO" ? <Bot size={11} className="text-[#f5d982]" /> : <UserCheck size={11} className="text-[#f5d982]" />}
      ...
      {view ? (
        <div className="mx-auto mt-1.5 grid max-w-xs grid-cols-2 gap-1 rounded-lg bg-[#211311]/70 p-1">
          <button type="button" disabled={!view.canManage || saving} onClick={() => void changeMode("AUTO")} className={cn("inline-flex items-center justify-center gap-1 rounded-md px-2 py-1.5 text-[9px] font-bold transition disabled:cursor-not-allowed", view.mode === "AUTO" ? "bg-[#f5d982] text-[#3d1f12]" : "text-white/65")}><Bot size={11} /> AI tự động</button>
...
export function AdminBookingCalendar({
  ...
  return <section className="relative overflow-hidden rounded-2xl border border-[#d8b46a]/60 bg-white shadow-[0_12px_32px_rgba(74,44,28,0.08)]">
    <div className="bg-gradient-to-r from-[#291714] via-[#63291d] to-[#d13f1f] px-3.5 py-3 text-white sm:px-5">
      <div className="grid grid-cols-[36px_minmax(0,1fr)_36px] items-center gap-2"><span className="flex h-9 w-9 items-center justify-center rounded-xl bg-white/10 text-[#f5d982] ring-1 ring-white/15"><CalendarClock size={17} /></span><div className="min-w-0 text-center"><p className="flex items-center justify-center gap-1 text-[9px] font-bold uppercase tracking-[0.13em] text-[#f5d982]">{session.role === "OWNER" ? <ShieldCheck size={11} /> : <Building2 size={11} />} {session.branchLabel}</p><h1 className="truncate text-base font-semibold sm:text-lg">Quản lý Booking</h1></div><button type="button" onClick={onCreate} className="flex h-9 w-9 items-center justify-center rounded-full bg-[#e3b23c] text-[#3d1f12]" aria-label="Tạo booking"><Plus size={16} /></button></div>
      <div className="mt-2.5 flex flex-wrap items-center justify-center gap-1.5 text-[9px] font-semibold"><span className="rounded-full bg-white/12 px-2.5 py-1">{monthRegular.length + monthBusiness.length} Booking</span><span className="rounded-full bg-[#fff0c9] px-2.5 py-1 text-[#805914]">{pending} chờ xử lý</span><span className="rounded-full bg-[#dff5e8] px-2.5 py-1 text-[#12683f]">{confirmed} đã xếp lịch</span></div>
    ...
    <div className="border-b border-[#eadbd1] bg-[#fffdfb] p-2.5 sm:px-4">
      <div className="grid grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-2"><button type="button" aria-label="Tháng trước" onClick={() => changeMonth(addMonths(visibleMonth, -1))} className="flex h-9 w-9 items-center justify-center rounded-xl border border-[#eadbd1] bg-white text-[#7a2318]"><ChevronLeft size={17} /></button><label className="relative block min-w-0"><span className="pointer-events-none absolute inset-0 flex items-center justify-center text-sm font-semibold capitalize text-[#291714]">{monthTitle(visibleMonth)}</span><input aria-label="Chọn tháng Booking" type="month" value={format(visibleMonth, "yyyy-MM")} onChange={(event) => event.target.value && changeMonth(new Date(`${event.target.value}-01T12:00:00`))} className="h-9 w-full cursor-pointer rounded-xl border border-[#eadbd1] bg-white text-center text-transparent [color-scheme:light]" /></label><button type="button" aria-label="Tháng sau" onClick={() => changeMonth(addMonths(visibleMonth, 1))} className="flex h-9 w-9 items-center justify-center rounded-xl border border-[#eadbd1] bg-white text-[#7a2318]"><ChevronRight size={17} /></button></div>
      <div className="scrollbar-hide mt-2 flex gap-1.5 overflow-x-auto rounded-xl bg-[#f7f1ed] p-1">{session.role === "OWNER" ? <button type="button" onClick={() => onBranchChange("all")} className={cn("shrink-0 rounded-lg px-3 py-2 text-[10px] font-semibold", allowedBranch === "all" ? "bg-[#291714] text-white shadow-sm" : "text-[#665b55]")}>Tất cả</button> : null}{branches.filter((item) => session.role === "OWNER" || item.id === session.branchId).map((branch) => { const count = regular.filter((item) => item.branchId === branch.id && isWithinInterval(new Date(item.timeIso), monthRange)).length + business.filter((item) => item.branchId === branch.id && isWithinInterval(new Date(item.startsAt), monthRange)).length; const active = allowedBranch === branch.id; return <button key={branch.id} type="button" onClick={() => session.role === "OWNER" && onBranchChange(branch.id)} className={cn("flex shrink-0 items-center gap-1.5 rounded-lg px-2.5 py-2 text-[10px] font-semibold", active ? "bg-[#d13f1f] text-white" : "text-[#665b55]")}><i className={cn("h-1.5 w-1.5 rounded-full", active ? "bg-[#f5d982]" : "bg-[#b9862c]")} />{branch.label}<span className={cn("rounded-full px-1.5 text-[8px]", active ? "bg-white/15" : "bg-white")}>{count}</span></button>; })}</div>
    </div>

    <div className="relative bg-gradient-to-b from-[#fffaf4] to-white px-2 pb-3 pt-5 sm:px-4 sm:pb-4"><div aria-hidden className="absolute left-4 right-4 top-0 flex -translate-y-1/2 justify-around">{Array.from({ length: 7 }, (_, index) => <span key={index} className="h-5 w-2 rounded-full border border-[#7b5129] bg-gradient-to-b from-[#f6d989] via-[#a8752e] to-[#f5d982] shadow-sm" />)}</div><div className="grid grid-cols-7 border-b border-[#eadbd1] pb-1.5 text-center text-[9px] font-bold uppercase tracking-[0.08em] text-[#8a6a58]">{WEEKDAYS.map((day) => <span key={day} className={day === "CN" ? "text-[#d13f1f]" : ""}>{day}</span>)}</div>
