  ...
  return (
    <div className="grid gap-4 lg:grid-cols-[0.9fr_1.1fr]">
      ...
      <section className="rounded-[2rem] border border-[#e7d6ca] bg-white p-5 shadow-xl">
        ...
        {event.status === "IN_SERVICE" ? <div className="mt-4 rounded-3xl bg-gradient-to-br from-[#4c191b] to-[#76551d] p-5 text-center text-white"><p className="flex items-center justify-center gap-1.5 text-xs uppercase tracking-wider text-white/70"><Clock3 size={15} /> {seconds !== null && seconds < 0 ? "Đang quá giờ" : "Còn lại"}</p><p className="mt-2 font-mono text-5xl font-bold tabular-nums">{time}</p><p className="mt-2 text-xs text-white/70">Chuông sẽ tự nhắc các bên khi đồng hồ về 00:00.</p></div> : null}
