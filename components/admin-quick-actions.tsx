...
function CompactPicker({
  ...
  return (
    <div className="relative text-xs font-semibold">
      ...
      >
        <span className="truncate">{selectedLabel}</span><ChevronDown size={14} className="shrink-0 text-[#9f1d20]" />
      </button>
      {expanded && !disabled ? (
        <div className="absolute inset-x-0 top-full z-30 mt-1 max-h-44 overflow-y-auto rounded-xl border border-[#e4d2c6] bg-white p-1.5 shadow-xl">
          {options.map((item) => (
            <button
              ...
              className={`block w-full rounded-lg px-2.5 py-2 text-left text-[11px] font-medium ${item.value === value ? "bg-[#fff0ed] text-[#9f1d20]" : "text-[#51443e] hover:bg-[#fffaf6]"}`}
            >
              {item.label}
            </button>
          ))}
        ...
        <div className="fixed inset-0 z-[80] flex items-center justify-center px-3 pb-[max(12px,env(safe-area-inset-bottom))] pt-[max(12px,env(safe-area-inset-top))] sm:p-5">
          ...
          <form onSubmit={submit} className="relative flex max-h-[calc(100dvh-24px)] w-full max-w-lg flex-col overflow-hidden rounded-3xl bg-white shadow-2xl">
            ...
            <div className="grid min-h-0 flex-1 gap-3 overflow-y-auto p-4 sm:grid-cols-2 sm:p-5">
              <label className="text-center text-xs font-semibold sm:col-span-2">Số tiền<input required inputMode="numeric" value={amount} onChange={(event) => setAmount(event.target.value)} placeholder="0" className="mt-1.5 w-full rounded-xl border border-[#d8b46a] bg-[#fffaf2] px-3 py-3 text-center text-xl font-bold text-[#9f1d20] outline-none focus:border-[#9f1d20]" />{amount ? <span className="mt-1 block text-[10px] font-normal text-[#8a7a72]">Ghi nhận vào Tổng chi: {formatMoney(Number(amount.replace(/\D/g, "")) || 0)}</span> : null}</label>
              ...
              <div className="sm:col-span-2">
                ...
                <button type="button" onClick={() => billInputRef.current?.click()} className="mt-1.5 flex w-full items-center gap-3 rounded-xl border border-dashed border-[#d4ad63] bg-[#fffaf2] p-3 text-left">
                  ...
                  {billPreview ? <img src={billPreview} alt="Bill đã tải" className="h-12 w-12 rounded-lg object-cover" /> : <span className="flex h-12 w-12 shrink-0 items-center justify-center rounded-lg bg-white text-[#9f1d20]"><ImagePlus size={20} /></span>}
                  ...
                  {aiStatus === "uploading" ? <Loader2 size={18} className="animate-spin text-[#9f1d20]" /> : aiStatus === "ready" || aiStatus === "manual" ? <FileCheck2 size={18} className="text-[#16784a]" /> : <ScanLine size={18} className="text-[#b9862c]" />}
                </button>
      ...
      {open && typeof document !== "undefined" ? createPortal((
        <div className="fixed inset-0 z-[80] flex items-end justify-center sm:items-center sm:p-5">
          ...
          <form onSubmit={submit} className="relative flex max-h-[calc(100dvh-12px)] w-full max-w-lg flex-col overflow-hidden rounded-t-[2rem] bg-white shadow-2xl sm:max-h-[calc(100dvh-40px)] sm:rounded-[2rem]">
            <div className="relative shrink-0 overflow-hidden bg-gradient-to-br from-[#231514] via-[#56291a] to-[#9f1d20] p-5 text-white">
