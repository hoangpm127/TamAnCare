  ...
  return (
    <>
      {trigger === "nav" ? (
        ...
        >
          <span className="flex h-12 w-12 items-center justify-center rounded-full border-[3px] border-[#fffdf9] bg-gradient-to-br from-[#d15d3f] to-[#8b2b28] text-white shadow-[0_7px_18px_rgba(124,41,39,0.32)] ring-1 ring-[#c64b32]/15 transition-transform active:scale-95">
            <Plus size={25} strokeWidth={2.6} />
          </span>
          <span className={`text-[11px] font-semibold transition-colors ${active ? "text-[#a63d2e]" : "text-[#826f66]"}`}>
            Đặt lịch
          </span>
        </button>
      ) : (
        <button
          type="button"
          onClick={() => setOpen(true)}
          className="mt-4 inline-flex items-center justify-center gap-1.5 rounded-full bg-gradient-to-b from-[#cc5a3b] to-[#a63d2e] px-6 py-2.5 text-sm font-semibold text-white shadow-lg shadow-black/25 ring-1 ring-white/15"
        >
      ...
      {open && typeof document !== "undefined" ? createPortal((
        <div className="fixed inset-0 z-[60] flex items-center justify-center px-4 pb-[max(16px,env(safe-area-inset-bottom))] pt-[max(16px,env(safe-area-inset-top))]">
          <button type="button" aria-label="Đóng" className="absolute inset-0 bg-[#281b18]/55 backdrop-blur-sm" onClick={() => setOpen(false)} />
          <div role="dialog" aria-modal="true" aria-labelledby="booking-choice-title" className="relative max-h-[92dvh] w-full max-w-sm overflow-y-auto rounded-[2rem] border border-[#d2ad5d]/55 bg-gradient-to-b from-[#fffdf9] to-[#fff8f2] px-4 pb-5 pt-5 shadow-[0_24px_70px_rgba(44,24,19,0.38)] ring-1 ring-white/70">
            {navigatingTo ? (
              <div className="absolute inset-0 z-30 flex flex-col items-center justify-center gap-2 bg-[#fdf8f3]/90 text-center backdrop-blur-sm" role="status" aria-live="polite">
                <Loader2 className="animate-spin text-[#c64b32]" size={27} />
            ...
            <div className="px-3 pb-3 text-center">
              <span className="inline-flex items-center gap-1.5 rounded-full bg-[#f2e3d4] px-2.5 py-1 text-[10px] font-semibold uppercase tracking-[0.12em] text-[#93352d]">
            ...
            <div className="mx-auto w-full max-w-[320px]">
              <div className="grid grid-cols-[1.05fr_0.95fr] grid-rows-2 gap-2.5">
                <button type="button" onClick={() => goTo("/booking")} className="group relative row-span-2 min-h-[230px] overflow-hidden rounded-[30px_16px_26px_16px] border border-[#e4bd9d] bg-gradient-to-br from-[#fff9f2] via-[#f8e2cf] to-[#efc8aa] p-4 text-left text-[#351d18] shadow-[0_12px_26px_rgba(91,48,31,0.14)] transition active:scale-[0.98]">
                  ...
                  <span className="pointer-events-none absolute bottom-10 right-9 h-2.5 w-2.5 rounded-full bg-[#9f7428]/35" />
                  <span className="relative flex h-full flex-col">
                    <span className="flex items-start justify-between gap-2">
                      <span className="flex h-12 w-12 -rotate-3 items-center justify-center rounded-[18px_12px_18px_12px] bg-white text-[#c64b32] shadow-sm ring-1 ring-[#c64b32]/10 transition group-hover:rotate-0"><UserRound size={21} /></span>
                      <span className="rounded-full bg-white/65 px-2 py-1 text-[8px] font-bold uppercase tracking-[0.12em] text-[#c64b32]">Riêng tư</span>
                    </span>
                    ...
                    <span className="mt-auto">
                      ...
                      <span className="mt-3 inline-flex items-center gap-1 text-[10px] font-semibold text-[#c64b32]">Chọn lịch <ArrowRight size={12} /></span>
                    </span>
                  </span>
                </button>

                <button type="button" onClick={() => goTo("/booking?invite=friend")} className="group relative min-h-[110px] overflow-hidden rounded-[16px_28px_16px_24px] bg-gradient-to-br from-[#d16a50] to-[#a63d2e] p-3 text-left text-white shadow-[0_10px_22px_rgba(122,41,39,0.16)] transition active:scale-[0.98]">
                ...
                <button type="button" onClick={() => goTo("/booking?invite=boss")} className="group relative min-h-[110px] overflow-hidden rounded-[26px_16px_28px_16px] bg-gradient-to-br from-[#7c2927] to-[#4c191b] p-3 text-left text-white shadow-[0_10px_22px_rgba(76,25,27,0.2)] transition active:scale-[0.98]">
                  <span className="pointer-events-none absolute -bottom-5 -left-4 h-16 w-16 rounded-full border-[10px] border-[#e7c878]/10" />
                  <span className="relative flex items-center gap-2.5">
                    <span className="flex h-10 w-10 -rotate-3 items-center justify-center rounded-[18px_12px_18px_12px] bg-white/10 text-[#e7c878] ring-1 ring-white/10 transition group-hover:rotate-0"><Handshake size={18} /></span>
              ...
              <button type="button" onClick={() => goTo("/doanh-nghiep")} className="group relative mt-2.5 flex min-h-[88px] w-full items-center gap-3 overflow-hidden rounded-[22px_30px_22px_30px] border border-[#c59a3d]/45 bg-gradient-to-r from-[#073d31] via-[#0b5d45] to-[#064233] px-4 py-3 text-left text-white shadow-[0_12px_26px_rgba(7,61,49,0.24)] transition active:scale-[0.99]">
                ...
                <span className="relative flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-[#e7c878] text-[#321d18] shadow-sm"><ArrowRight size={14} /></span>
              </button>
            </div>

            <div className="mt-4 flex items-center justify-center gap-1.5 text-center text-[11px] text-[#826f66]">
              <Users size={12} className="shrink-0 text-[#c64b32]" /> Hơn 500 khách hàng & doanh nghiệp đã tin dùng.
            </div>
          </div>
        </div>
      ), document.body) : null}
