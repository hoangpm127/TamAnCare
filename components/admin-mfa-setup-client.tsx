...
export function AdminMfaSetupClient({ displayName, alreadyEnabled, landingPath = "/admin" }: { displayName: string; alreadyEnabled: boolean; landingPath?: string }) {
  ...
  if (alreadyEnabled && recoveryCodes.length === 0) {
    return (
      <section className="w-full max-w-lg rounded-3xl bg-[#fffaf6] p-6 text-center shadow-2xl ring-1 ring-white/10">
        ...
        <button type="button" onClick={() => router.replace(landingPath)} className="mt-5 rounded-full bg-[#9f1d20] px-6 py-3 text-sm font-semibold text-white">Vào Trung tâm quản trị</button>
      </section>
    );
  }
  ...
  if (recoveryCodes.length > 0) {
    return (
      <section className="w-full max-w-xl rounded-3xl bg-[#fffaf6] p-5 shadow-2xl ring-1 ring-white/10 sm:p-7">
        ...
        <button type="button" onClick={() => void copyCodes()} className="mt-3 flex w-full items-center justify-center gap-2 rounded-full border border-[#9f1d20] px-5 py-2.5 text-xs font-semibold text-[#9f1d20]"><Copy size={14} /> Sao chép mã khôi phục</button>
        <label className="mt-3 flex items-start gap-2 text-xs leading-5 text-[#554842]"><input type="checkbox" checked={savedCodes} onChange={(event) => setSavedCodes(event.target.checked)} className="mt-1 h-4 w-4 accent-[#9f1d20]" /> Tôi đã lưu mã khôi phục ngoài thiết bị này.</label>
        <button type="button" disabled={!savedCodes} onClick={() => { router.replace(landingPath); router.refresh(); }} className="mt-4 w-full rounded-full bg-[#9f1d20] px-5 py-3 text-sm font-semibold text-white disabled:opacity-40">Tiếp tục vào quản trị</button>
      </section>
    );
  }
  ...
  return (
    <section className="w-full max-w-xl rounded-3xl bg-[#fffaf6] p-5 shadow-2xl ring-1 ring-white/10 sm:p-7">
      <div className="flex items-start gap-3"><span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full bg-[#fff0ed] text-[#9f1d20]"><ShieldCheck size={22} /></span><div><p className="text-[10px] font-semibold uppercase tracking-[0.14em] text-[#9f1d20]">Bảo vệ tài khoản quản trị</p><h1 className="mt-1 text-xl font-semibold">Thiết lập Authenticator</h1><p className="mt-1 text-xs leading-5 text-[#786a63]">Mã thay đổi mỗi 30 giây và nằm ngoài mật khẩu.</p></div></div>
      ...
      {!setup ? (
        <form onSubmit={beginSetup} className="mt-5 space-y-3">
          ...
          <button disabled={submitting} className="flex w-full items-center justify-center gap-2 rounded-full bg-[#9f1d20] px-5 py-3 text-sm font-semibold text-white disabled:opacity-60">{submitting ? <Loader2 className="animate-spin" size={16} /> : <KeyRound size={16} />} Bắt đầu thiết lập</button>
        </form>
      ) : (
        <form onSubmit={confirmSetup} className="mt-5">
          ...
          <div className="mx-auto mt-3 flex h-[260px] w-[260px] items-center justify-center rounded-2xl bg-white p-2 ring-1 ring-[#eadbd1]">
            ...
            {qrDataUrl ? <img src={qrDataUrl} alt="QR thiết lập Authenticator" className="h-full w-full" /> : <Loader2 className="animate-spin text-[#9f1d20]" />}
          </div>
          ...
          <label className="mt-4 block text-center text-xs font-semibold">2. Nhập mã 6 số đang hiển thị
            <input required inputMode="numeric" pattern="[0-9]{6}" maxLength={6} value={code} onChange={(event) => setCode(event.target.value.replace(/\D/g, ""))} className="mx-auto mt-2 block w-48 rounded-xl border border-[#eadbd1] bg-white px-3 py-3 text-center font-mono text-xl tracking-[0.35em] outline-none focus:border-[#9f1d20]" />
          </label>
          {error ? <p role="alert" className="mt-3 rounded-xl bg-red-50 p-3 text-xs text-red-700">{error}</p> : null}
          <button disabled={submitting} className="mt-4 flex w-full items-center justify-center gap-2 rounded-full bg-[#9f1d20] px-5 py-3 text-sm font-semibold text-white disabled:opacity-60">{submitting ? <Loader2 className="animate-spin" size={16} /> : <ShieldCheck size={16} />} Xác nhận và bật MFA</button>
        </form>
      )}
    </section>
  );
