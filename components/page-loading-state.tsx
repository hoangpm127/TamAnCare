    ...
    <div className="route-loading-state relative mx-auto w-full max-w-7xl px-4 py-5 sm:px-6 lg:px-10" role="status" aria-live="polite">
      ...
      <div className="route-loading-content">
        <div
          className={cn(
            ...
            adminLike
              ...
              : "border border-[#e7d6ca] bg-white",
          )}
        >
          <div className="flex items-center gap-3">
            <span className={cn("route-skeleton h-11 w-11 shrink-0 rounded-2xl", adminLike ? "bg-white/[0.16]" : "bg-[#f3e5de]")} />
            <div className="min-w-0 flex-1 space-y-2">
              <div className={cn("route-skeleton h-4 w-2/5 rounded-full", adminLike ? "bg-white/[0.18]" : "bg-[#e7d6ca]")} />
        ...
        <div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-4">
          {[0, 1, 2, 3].map((item) => (
            <div key={item} className="rounded-2xl border border-[#e7d6ca] bg-white p-4 shadow-sm">
        ...
        <div className="mt-4 space-y-3 rounded-3xl border border-[#e7d6ca] bg-white p-4 shadow-sm">
          {[0, 1, 2].map((item) => (
            <div key={item} className="flex items-center gap-3 border-b border-[#f2e8e2] pb-3 last:border-0 last:pb-0">
              ...
              <div className="min-w-0 flex-1 space-y-2">
                <div className="route-skeleton h-3 w-3/5 rounded-full bg-[#e7d6ca]" />
