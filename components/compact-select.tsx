      ...
      <button
        ...
        className={cn(
          ...
          dark
            ...
            : "border-[#e7d6ca] bg-white text-[#2d211d] shadow-[0_1px_4px_rgba(92,58,30,0.04)] hover:border-[#d2ad5d]",
          triggerClassName,
        )}
      >
        {icon ? <span className={cn("shrink-0", dark ? "text-[#e7c878]" : "text-[#c64b32]")}>{icon}</span> : null}
        <span className="min-w-0 flex-1 truncate">{selected?.label ?? placeholder}</span>
        <ChevronDown size={15} className={cn("shrink-0 transition", open && "rotate-180", dark ? "text-white/65" : "text-[#826f66]")} />
      </button>
      ...
      {open && position && typeof document !== "undefined"
        ? createPortal(
            <div className="fixed inset-0 z-[160]">
              ...
              <section
                className="fixed overflow-hidden rounded-2xl border border-[#d2ad5d]/70 bg-[#fffdfb] shadow-[0_16px_42px_rgba(41,20,13,0.28)]"
                style={{ left: position.left, top: position.top, width: position.width, maxHeight: position.maxHeight }}
              >
                <header className="flex items-center gap-2 border-b border-[#e7d6ca] bg-gradient-to-r from-[#2f1b16] via-[#4a281e] to-[#762b27] px-3 py-2.5 text-white">
                  <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-white/10 text-[#e7c878] ring-1 ring-white/15">
                    <SlidersHorizontal size={14} />
                  </span>
                  <div className="min-w-0 flex-1">
                    <BrandWordmark className="h-[10px] w-[74px] text-[#e7c878]" />
                      ...
                      return (
                        <button
                          ...
                          className={cn(
                            ...
                            active
                              ? "border-[#c64b32] bg-gradient-to-br from-[#f8ebe5] to-[#fff8e8] text-[#8f171b] shadow-sm"
                              : "border-[#eee3dc] bg-white text-[#33251f] hover:border-[#d2ad5d] hover:bg-[#fdf8f3]",
                          )}
                        >
                          <span className={cn("flex h-6 w-6 shrink-0 items-center justify-center rounded-full border", active ? "border-[#c64b32] bg-[#c64b32] text-white" : "border-[#e3d5cc] bg-[#fbf6f2] text-transparent")}>
                            <Check size={12} />
                          </span>
                          <span className="min-w-0 flex-1">
                            <span className="block text-[12px] font-semibold leading-4">{option.label}</span>
                            {option.description ? <span className="mt-0.5 block text-[9px] leading-3.5 text-[#826f66]">{option.description}</span> : null}
                          </span>
                        </button>
                      );
                    })}
