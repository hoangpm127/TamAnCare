...
const statusMeta: Record<RefundStatus, { label: string; tone: string; icon: typeof Clock3 }> = {
  REQUESTED: { label: "Chờ duyệt", tone: "border-[#e8d39e] bg-[#fff8e8] text-[#805914]", icon: Clock3 },
  APPROVED: { label: "Đã duyệt · chờ chuyển", tone: "border-[#b9d8e8] bg-[#edf7fc] text-[#236580]", icon: BadgeCheck },
  COMPLETED: { label: "Đã hoàn tiền", tone: "border-[#b8dfc9] bg-[#edf9f2] text-[#16784a]", icon: CheckCircle2 },
  REJECTED: { label: "Đã từ chối", tone: "border-[#efc6be] bg-[#fff2ef] text-[#9f1d20]", icon: XCircle },
...
export function AdminRefundCenter() {
  ...
  if (session && !["OWNER", "BRANCH_MANAGER"].includes(session.role)) {
    return <main className="mx-auto max-w-xl px-4 py-12 text-center"><ShieldCheck className="mx-auto text-[#9f1d20]" /><h1 className="mt-3 text-lg font-semibold">Khu vực nghiệp vụ có kiểm soát</h1><p className="mt-2 text-sm text-[#8a7a72]">Chỉ Chủ Tâm An và Quản lý cơ sở được truy cập quy trình hoàn tiền.</p><Link href="/admin" className="mt-5 inline-flex items-center gap-2 rounded-full bg-[#9f1d20] px-4 py-2 text-sm font-semibold text-white"><ArrowLeft size={15} /> Về trang quản trị</Link></main>;
  }
  ...
  return (
    <main className="mx-auto max-w-7xl px-3 py-3 sm:px-6 sm:py-5 lg:px-10">
      <section className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-[#201413] via-[#4c281e] to-[#8f151a] p-4 text-white shadow-xl sm:p-5">
        <div className="pointer-events-none absolute -right-10 -top-14 h-48 w-48 rounded-full bg-[#f0c65e]/20 blur-3xl" />
        <div className="relative flex items-start gap-3">
          <span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-white/10 text-[#f5d982] ring-1 ring-white/15"><RotateCcw size={21} /></span>
          <div className="min-w-0 flex-1"><p className="text-[10px] font-semibold uppercase tracking-[0.16em] text-[#f5d982]">Kiểm soát hai lớp · Sổ cái thật</p><h1 className="mt-0.5 text-xl font-semibold">Trung tâm hoàn tiền</h1><p className="mt-1 max-w-2xl text-[11px] leading-4 text-white/70">Quản lý lập yêu cầu · Chủ khác phê duyệt · chỉ hạch toán khi đã có mã chuyển khoản ngân hàng.</p></div>
        ...
        <div className="relative mt-4 grid grid-cols-3 gap-2 border-t border-white/15 pt-3 text-center">
          ...
          <span className="rounded-xl bg-white/[0.07] p-2 text-[9px] text-white/55">Đã hoàn<strong className="mt-0.5 block truncate text-sm text-[#f5d982]">{formatMoney(counts.completedAmount)}</strong></span>
        </div>
      </section>

      {message ? <div role="status" className={cn("mt-3 rounded-xl border px-3 py-2.5 text-xs leading-5", message.tone === "ok" ? "border-[#b8dfc9] bg-[#edf9f2] text-[#16784a]" : "border-[#efc6be] bg-[#fff2ef] text-[#9f1d20]")}>{message.text}</div> : null}

      <div className="mt-3 grid gap-3 lg:grid-cols-[0.78fr_1.22fr]">
        <form onSubmit={submitRequest} className="h-fit rounded-2xl border border-[#d8b46a]/60 bg-white p-4 shadow-sm">
          <div className="flex items-start gap-3"><span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-[#fff2ef] text-[#9f1d20]"><ReceiptText size={18} /></span><div><h2 className="text-sm font-semibold">Lập yêu cầu hoàn tiền</h2><p className="mt-0.5 text-[10px] leading-4 text-[#8a7a72]">Hạn mức tính từ khoản thu dịch vụ/cọc gốc; không bao gồm phần Tip KTV.</p></div></div>
          <div className="mt-4 space-y-3">
            ...
            {selectedSource ? <div className="grid grid-cols-2 gap-2 rounded-xl bg-[#fffaf6] p-3 text-center text-[10px] text-[#8a7a72]"><span>Khoản thu gốc<strong className="mt-1 block text-xs text-[#191414]">{formatMoney(selectedSource.amount)}</strong></span><span>Còn có thể hoàn<strong className="mt-1 block text-xs text-[#16784a]">{formatMoney(selectedSource.refundableAmount)}</strong></span></div> : <p className="rounded-xl border border-dashed border-[#eadbd1] p-4 text-center text-[10px] text-[#8a7a72]">Không có khoản thu đủ điều kiện trong phạm vi của bạn.</p>}
            <label className="block text-[10px] font-semibold uppercase tracking-[0.1em] text-[#7c6256]">Số tiền hoàn
              <input inputMode="numeric" value={amount} onChange={(event) => setAmount(event.target.value.replace(/\D/g, ""))} max={selectedSource?.refundableAmount} placeholder="0" className="mt-1.5 w-full rounded-xl border border-[#eadbd1] bg-white px-3 py-3 text-center text-lg font-semibold outline-none focus:border-[#9f1d20]" />
              {amount ? <span className="mt-1 block text-center text-[10px] font-normal normal-case tracking-normal text-[#16784a]">{formatMoney(Number(amount))}</span> : null}
            </label>
            <label className="block text-[10px] font-semibold uppercase tracking-[0.1em] text-[#7c6256]">Lý do và căn cứ
              <textarea value={reason} onChange={(event) => setReason(event.target.value)} minLength={10} maxLength={500} rows={3} placeholder="Ví dụ: Khách hủy đúng điều kiện chính sách, hoàn lại tiền cọc…" className="mt-1.5 w-full resize-none rounded-xl border border-[#eadbd1] bg-white px-3 py-2.5 text-xs font-normal normal-case tracking-normal outline-none focus:border-[#9f1d20]" />
            </label>
            <button disabled={busy === "create" || !selectedSource || !amount || reason.trim().length < 10 || Number(amount) > (selectedSource?.refundableAmount ?? 0)} className="flex w-full items-center justify-center gap-2 rounded-xl bg-[#9f1d20] px-4 py-3 text-xs font-semibold text-white disabled:cursor-not-allowed disabled:opacity-45">{busy === "create" ? <LoaderCircle size={15} className="animate-spin" /> : <UserRoundCheck size={15} />} Gửi Chủ phê duyệt</button>
            <p className="text-center text-[9px] leading-4 text-[#8a7a72]">Lập yêu cầu không đồng nghĩa đã trả tiền. Người lập không được tự phê duyệt.</p>
          </div>
        </form>

        <section className="min-w-0 rounded-2xl border border-[#d8b46a]/60 bg-white p-3 shadow-sm sm:p-4">
          <div className="flex items-start justify-between gap-3"><div><h2 className="text-sm font-semibold">Sổ yêu cầu hoàn tiền</h2><p className="mt-0.5 text-[10px] text-[#8a7a72]">{data?.requests.length ?? 0} yêu cầu · đầy đủ người lập, người duyệt và mã ngân hàng</p></div><Landmark size={18} className="text-[#9f1d20]" /></div>
          <div className="scrollbar-hide mt-3 flex gap-1.5 overflow-x-auto pb-1">
            {(["ALL", "REQUESTED", "APPROVED", "COMPLETED", "REJECTED", "CANCELLED"] as const).map((status) => <button type="button" key={status} onClick={() => setFilter(status)} className={cn("shrink-0 rounded-full border px-3 py-1.5 text-[10px] font-semibold", filter === status ? "border-[#9f1d20] bg-[#9f1d20] text-white" : "border-[#eadbd1] bg-[#fffaf6] text-[#665b55]")}>{status === "ALL" ? "Tất cả" : statusMeta[status].label}</button>)}
          </div>
          <div className="mt-3 space-y-2.5">
            {loading ? <div className="flex min-h-32 items-center justify-center text-[#9f1d20]"><LoaderCircle className="animate-spin" /></div> : null}
            {!loading && !filtered.length ? <p className="rounded-xl border border-dashed border-[#eadbd1] p-8 text-center text-xs text-[#8a7a72]">Chưa có yêu cầu phù hợp bộ lọc.</p> : null}
            {filtered.map((item) => {
              const meta = statusMeta[item.status];
              const StatusIcon = meta.icon;
              const ownRequest = item.requestedById === data?.actorId;
              return <article key={item.id} className="rounded-2xl border border-[#eadbd1] bg-[#fffdfb] p-3.5">
                <div className="flex items-start justify-between gap-2"><div className="min-w-0"><p className="truncate text-xs font-semibold">{item.customerName} · {displayBookingCode(item.referenceCode)}</p><p className="mt-0.5 flex items-center gap-1 text-[9px] text-[#8a7a72]"><Building2 size={10} /> {item.branchName} · {dateTime(item.createdAt)}</p></div><span className={cn("inline-flex shrink-0 items-center gap-1 rounded-full border px-2 py-1 text-[9px] font-semibold", meta.tone)}><StatusIcon size={10} /> {meta.label}</span></div>
                <div className="mt-3 grid grid-cols-3 rounded-xl bg-white p-2.5 text-center text-[9px] text-[#8a7a72] ring-1 ring-[#eee3dc]"><span>Nguồn thu<strong className="mt-0.5 block text-[10px] text-[#191414]">{item.sourceType === "DEPOSIT" ? "Tiền cọc" : "Dịch vụ"}</strong></span><span>Thu gốc<strong className="mt-0.5 block text-[10px] text-[#191414]">{formatMoney(item.sourceAmount)}</strong></span><span>Đề nghị hoàn<strong className="mt-0.5 block text-[11px] text-[#9f1d20]">{formatMoney(item.amount)}</strong></span></div>
                <p className="mt-2.5 rounded-xl bg-[#fffaf6] px-3 py-2 text-[10px] leading-4 text-[#665b55]"><strong className="text-[#3b2c26]">Lý do:</strong> {item.reason}</p>
                <div className="mt-2 grid gap-1 text-[9px] leading-4 text-[#8a7a72] sm:grid-cols-2"><span>Người lập: <strong className="text-[#4c3c34]">{item.requestedByName}</strong></span>{item.approvedByName ? <span>Người duyệt: <strong className="text-[#4c3c34]">{item.approvedByName}</strong></span> : null}{item.completedByName ? <span>Ghi nhận chuyển: <strong className="text-[#4c3c34]">{item.completedByName}</strong></span> : null}{item.bankReference ? <span>Mã ngân hàng: <strong className="text-[#16784a]">{item.bankReference}</strong></span> : null}</div>
                {item.approvalNote ? <p className="mt-1.5 text-[9px] italic leading-4 text-[#806e65]">Ghi chú xử lý: {item.approvalNote}</p> : null}

                {item.status === "REQUESTED" && data?.role === "OWNER" && ownRequest ? <p className="mt-3 rounded-xl border border-[#e8d39e] bg-[#fff8e8] px-3 py-2 text-[10px] leading-4 text-[#805914]">Bạn là người lập nên không thể tự duyệt. Hãy dùng một tài khoản Chủ độc lập để kiểm tra.</p> : null}
                {item.status === "REQUESTED" ? <div className="mt-3 space-y-2"><input value={notes[item.id] ?? ""} onChange={(event) => setNotes((current) => ({ ...current, [item.id]: event.target.value }))} placeholder={data?.role === "OWNER" && !ownRequest ? "Ghi chú duyệt hoặc lý do từ chối" : "Ghi chú hủy yêu cầu (không bắt buộc)"} className="w-full rounded-xl border border-[#eadbd1] bg-white px-3 py-2 text-[10px] outline-none focus:border-[#9f1d20]" /><div className="grid grid-cols-2 gap-2">{data?.role === "OWNER" && !ownRequest ? <><button type="button" disabled={Boolean(busy)} onClick={() => void transition(item, "APPROVE")} className="flex items-center justify-center gap-1.5 rounded-xl bg-[#16784a] px-3 py-2 text-[10px] font-semibold text-white"><BadgeCheck size={13} /> Phê duyệt</button><button type="button" disabled={Boolean(busy) || (notes[item.id]?.trim().length ?? 0) < 3} onClick={() => void transition(item, "REJECT")} className="flex items-center justify-center gap-1.5 rounded-xl border border-[#9f1d20] px-3 py-2 text-[10px] font-semibold text-[#9f1d20]"><XCircle size={13} /> Từ chối</button></> : null}{(data?.role === "OWNER" || ownRequest) ? <button type="button" disabled={Boolean(busy)} onClick={() => void transition(item, "CANCEL")} className={cn("flex items-center justify-center gap-1.5 rounded-xl border border-[#d7ccc5] px-3 py-2 text-[10px] font-semibold text-[#665b55]", data?.role === "OWNER" && !ownRequest ? "col-span-2" : "col-span-2")}><Ban size={13} /> Hủy yêu cầu</button> : null}</div></div> : null}
