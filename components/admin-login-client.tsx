...
export function AdminLoginClient() {
  ...
  return (
    <main className="min-h-screen bg-[#241514] px-3 py-3 text-[#281b18] sm:px-6 sm:py-8">
      ...
      <div className="relative mx-auto max-w-4xl">
        ...
        <section className="overflow-hidden rounded-2xl border border-white/10 bg-[#fdf8f3] shadow-2xl sm:rounded-3xl">
          <div className="grid lg:grid-cols-[0.82fr_1.18fr]">
            <div className="bg-gradient-to-br from-[#8b2b28] via-[#5c1014] to-[#2a1513] p-5 text-white lg:p-7">
              <div className="flex items-center gap-3">
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img src="/icon-64.png" alt="" className="h-10 w-10 rounded-full ring-1 ring-[#c59a3d]" />
                <div><BrandWordmark className="h-[18px] w-[132px] text-[#e8cf8a]" /><p className="mt-0.5 text-[10px] text-white/65">Vận hành & đầu tư</p></div>
              </div>
              <p className="mt-6 text-[10px] font-semibold uppercase tracking-[0.16em] text-[#e7c878]">Khu vực được bảo vệ</p>
              ...
              <div className="mt-5 space-y-2.5 text-xs text-white/75">
                {["Không lưu mật khẩu trên trình duyệt", "Tự khóa khi đăng nhập sai nhiều lần", "Phiên đăng nhập được mã hóa và giới hạn thời gian"].map((item) => (
                  <p key={item} className="flex items-center gap-2"><Check size={14} className="text-[#e7c878]" /> {item}</p>
                ))}
              </div>
            </div>
            ...
            <div className="p-5 lg:p-7">
              ...
              <form onSubmit={signIn} className="mt-6 space-y-3">
                {mfaRequired ? (
                  <div className="rounded-2xl bg-[#fbf2e7] p-4 ring-1 ring-[#e7d6ca]">
                    <span className="mx-auto flex h-10 w-10 items-center justify-center rounded-full bg-white text-[#c64b32] ring-1 ring-[#e7d6ca]"><KeyRound size={19} /></span>
                    ...
                    <label className="mt-3 block">
                      ...
                      <input required autoFocus value={mfaCode} onChange={(event) => setMfaCode(event.target.value.toUpperCase())} autoComplete="one-time-code" inputMode="text" maxLength={40} className="mt-1.5 w-full rounded-xl border border-[#e7d6ca] bg-white px-3 py-3 text-center font-mono text-lg tracking-[0.2em] outline-none focus:border-[#c64b32]" placeholder="000000" />
                    </label>
                    <button type="button" onClick={() => { setMfaRequired(false); setMfaCode(""); setPassword(""); setError(""); }} className="mt-2 w-full text-center text-[11px] font-semibold text-[#c64b32]">Dùng tài khoản khác</button>
                  </div>
                ) : (
                  <>
                ...
                <label className="block">
                  ...
                  <span className="mt-1.5 flex items-center gap-2 rounded-xl border border-[#e7d6ca] bg-white px-3 focus-within:border-[#c64b32]">
                    <LockKeyhole size={16} className="shrink-0 text-[#826f66]" />
                    ...
                    <button type="button" onClick={() => setShowPassword((value) => !value)} className="text-[#826f66]" aria-label={showPassword ? "Ẩn mật khẩu" : "Hiện mật khẩu"}>{showPassword ? <EyeOff size={17} /> : <Eye size={17} />}</button>
                  </span>
                </label>
                  </>
                )}
                {error ? <p role="alert" className="rounded-xl bg-red-50 px-3 py-2.5 text-xs font-medium text-red-700">{error}</p> : null}
                <button type="submit" disabled={submitting} aria-busy={submitting} className="flex w-full items-center justify-center gap-2 rounded-full bg-gradient-to-b from-[#b6403a] to-[#8b2b28] px-5 py-3 text-sm font-semibold text-white shadow-md shadow-[#5c1014]/20 disabled:opacity-60">
                  {submitting ? <Loader2 size={16} className="animate-spin" /> : null}
                  {submitting ? "Đang xác thực…" : mfaRequired ? "Xác nhận mã bảo mật" : "Đăng nhập an toàn"}
                </button>
              </form>
              <p className="mt-3 text-center text-[10px] leading-4 text-[#826f66]">Tài khoản bị tạm khóa 15–30 phút khi đăng nhập sai liên tiếp.</p>
            </div>
          </div>
        </section>
      </div>
