      ...
      <div className="mt-3 space-y-3">
        {(payload?.branches ?? []).map((branch) => {
          ...
          return <section key={branch.id} className="rounded-2xl border border-[#d8b46a]/50 bg-white p-3 shadow-sm">
            <div className="mb-2.5 flex items-center justify-between gap-3"><div><h2 className="text-sm font-semibold">{branch.label}</h2><p className="text-[9px] text-[#8a7a72]">{therapists.filter((item) => item.liveStatus === "AVAILABLE").length}/{therapists.length} KTV rảnh ngay</p></div>{gap ? <span className="rounded-full bg-[#fff2ef] px-2.5 py-1 text-[9px] font-bold text-[#9f1d20]">Business cần gán thêm {gap} KTV</span> : null}</div>
