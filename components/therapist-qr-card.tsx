...
export function TherapistQrCard({ dataUrl, therapistName, branchLabel, compact = false }: { dataUrl: string; therapistName: string; branchLabel: string; compact?: boolean }) {
  return (
    <section className="overflow-hidden rounded-2xl border border-[#c4ddd2] bg-gradient-to-br from-[#effaf3] to-white p-4 shadow-sm">
      <div className={compact ? "text-center" : "grid items-center gap-4 sm:grid-cols-[190px_1fr]"}>
        <div className="mx-auto w-fit rounded-2xl border border-[#dbe9e2] bg-white p-2 shadow-sm">
        ...
        <div className={compact ? "mt-3" : "min-w-0"}>
          <p className="flex items-center justify-center gap-1.5 text-[10px] font-bold uppercase tracking-[0.14em] text-[#0b5d45] sm:justify-start"><QrCode size={13} /> QR KTV riêng</p>
          ...
          <a href={dataUrl} download={`taman-ktv-${therapistName.toLocaleLowerCase("vi").replace(/\s+/g, "-")}.png`} className="mt-3 inline-flex items-center justify-center gap-1.5 rounded-full bg-[#0b5d45] px-4 py-2.5 text-xs font-semibold text-white"><Download size={14} /> Tải QR KTV</a>
        </div>
      </div>
    </section>
  );
