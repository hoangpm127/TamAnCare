            ...
            <article key={customer.id} className="relative overflow-hidden rounded-2xl border border-[#d8b46a]/60 bg-white shadow-sm">
              ...
              {isOpen ? <div className="border-t border-[#eadbd1] bg-[#fffdfb] p-3.5">
                <div className="grid gap-2 sm:grid-cols-2">
                  <div className="rounded-xl border border-[#eadbd1] bg-white p-3"><p className="flex items-center gap-1.5 text-xs font-semibold"><Sparkles size={13} className="text-[#d13f1f]" /> Nhu cầu & ghi chú</p><p className="mt-1.5 text-[11px] leading-4 text-[#665b55]">{customer.note}</p></div>
                  <div className="rounded-xl border border-[#eadbd1] bg-white p-3"><p className="flex items-center gap-1.5 text-xs font-semibold"><CircleDollarSign size={13} className="text-[#d13f1f]" /> Giá trị khách hàng</p><div className="mt-2 grid grid-cols-2 gap-2 text-[10px]"><span className="rounded-lg bg-[#fff7ec] p-2">Trung bình/lượt<strong className="mt-0.5 block text-xs">{formatMoney(Math.round(customer.totalSpend / Math.max(1, customer.totalVisits)))}</strong></span><span className="rounded-lg bg-[#fff7ec] p-2">Nhóm chăm sóc<strong className="mt-0.5 block text-xs">{segmentLabel(customer.segment)}</strong></span></div></div>
                  {detailMode === "full" ? <div className="rounded-xl border border-[#eadbd1] bg-white p-3 sm:col-span-2"><p className="flex items-center gap-1.5 text-xs font-semibold"><CalendarClock size={13} className="text-[#d13f1f]" /> Lịch sử phục vụ gần đây</p><div className="mt-2 space-y-2">{customerBookings.length === 0 ? <p className="text-[10px] text-[#8a7a72]">Chưa có lịch sử.</p> : customerBookings.map((booking) => <div key={booking.id} className="rounded-lg bg-[#fffaf6] p-2.5"><div className="flex justify-between gap-2"><p className="text-[10px] font-semibold">{booking.service.name}</p><span className="text-[9px] text-[#8a7a72]">{new Date(booking.startTime).toLocaleDateString("vi-VN")}</span></div><p className="mt-1 text-[9px] text-[#665b55]">{booking.therapist?.fullName ?? "Cơ sở sắp xếp"} · {formatMoney(booking.totalAmount)} · {booking.status === "COMPLETED" ? "Hoàn thành" : "Đã đặt"}</p></div>)}</div></div> : null}
                </div>
                <Link href={`/admin/customers/${customer.id}`} className="mt-3 inline-flex w-full items-center justify-center gap-1.5 rounded-full bg-[#d13f1f] py-2.5 text-xs font-semibold text-white">Mở hồ sơ đầy đủ <ExternalLink size={13} /></Link>
              </div> : null}
            </article>
          );
        })}
