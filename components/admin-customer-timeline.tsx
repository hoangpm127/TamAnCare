          ...
          return (
            <article key={customer.id} className="relative overflow-hidden rounded-2xl border border-[#d2ad5d]/60 bg-white shadow-sm">
              <button type="button" onClick={() => setExpanded(isOpen ? null : customer.id)} className="w-full p-3.5 text-left">
                <div className="flex items-start gap-3"><span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-[#f8ebe5] text-[#c64b32]"><UserRound size={18} /></span><div className="min-w-0 flex-1"><div className="flex items-start justify-between gap-2"><div><h2 className="text-sm font-semibold">{customer.fullName}</h2><p className="mt-0.5 text-[10px] text-[#826f66]">{customer.phone} · {branches.find((branch) => branch.id === latestBranchId)?.label ?? "Toàn hệ thống"}</p></div><span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-[#fbf2e7] text-[#7a3e1d]">{isOpen ? <ChevronDown size={15} /> : <ChevronRight size={15} />}</span></div><div className="mt-2 flex flex-wrap gap-1.5">{customerGroups(customer).slice(0, 2).map((group) => <span key={group} className="rounded-full bg-[#fff7df] px-2 py-1 text-[9px] font-semibold text-[#76551d]">{segmentLabel(group)}</span>)}<span className="rounded-full bg-[#f4f0ed] px-2 py-1 text-[9px] text-[#68574f]">Ghé gần nhất {customer.lastVisit}</span></div></div></div>
                <div className="mt-3 grid grid-cols-3 gap-1.5 text-center"><div className="rounded-lg bg-[#fdf8f3] py-2"><strong className="block text-xs">{customer.totalVisits}</strong><span className="text-[9px] text-[#826f66]">Lượt ghé</span></div><div className="rounded-lg bg-[#fdf8f3] py-2"><strong className="block text-xs">{formatMoney(customer.totalSpend)}</strong><span className="text-[9px] text-[#826f66]">Chi tiêu</span></div><div className="rounded-lg bg-[#fdf8f3] py-2"><strong className="block truncate px-1 text-xs">{customer.favoriteTherapist}</strong><span className="text-[9px] text-[#826f66]">KTV quen</span></div></div>
              </button>

              {isOpen ? <div className="border-t border-[#e7d6ca] bg-[#fffdfb] p-3.5">
                <div className="grid gap-2 sm:grid-cols-2">
                  <div className="rounded-xl border border-[#e7d6ca] bg-white p-3"><p className="flex items-center gap-1.5 text-xs font-semibold"><Sparkles size={13} className="text-[#c64b32]" /> Nhu cầu & ghi chú</p><p className="mt-1.5 text-[11px] leading-4 text-[#68574f]">{customer.note}</p></div>
                  <div className="rounded-xl border border-[#e7d6ca] bg-white p-3"><p className="flex items-center gap-1.5 text-xs font-semibold"><CircleDollarSign size={13} className="text-[#c64b32]" /> Giá trị khách hàng</p><div className="mt-2 grid grid-cols-2 gap-2 text-[10px]"><span className="rounded-lg bg-[#fbf2e7] p-2">Trung bình/lượt<strong className="mt-0.5 block text-xs">{formatMoney(Math.round(customer.totalSpend / Math.max(1, customer.totalVisits)))}</strong></span><span className="rounded-lg bg-[#fbf2e7] p-2">Nhóm chăm sóc<strong className="mt-0.5 block text-xs">{segmentLabel(customer.segment)}</strong></span></div></div>
                  {detailMode === "full" ? <div className="rounded-xl border border-[#e7d6ca] bg-white p-3 sm:col-span-2"><p className="flex items-center gap-1.5 text-xs font-semibold"><CalendarClock size={13} className="text-[#c64b32]" /> Lịch sử phục vụ gần đây</p><div className="mt-2 space-y-2">{customerBookings.length === 0 ? <p className="text-[10px] text-[#826f66]">Chưa có lịch sử.</p> : customerBookings.map((booking) => <div key={booking.id} className="rounded-lg bg-[#fdf8f3] p-2.5"><div className="flex justify-between gap-2"><p className="text-[10px] font-semibold">{booking.service.name}</p><span className="text-[9px] text-[#826f66]">{new Date(booking.startTime).toLocaleDateString("vi-VN")}</span></div><p className="mt-1 text-[9px] text-[#68574f]">{booking.therapist?.fullName ?? "Cơ sở sắp xếp"} · {formatMoney(booking.totalAmount)} · {booking.status === "COMPLETED" ? "Hoàn thành" : "Đã đặt"}</p></div>)}</div></div> : null}
                </div>
                <Link href={`/admin/customers/${customer.id}`} className="mt-3 inline-flex w-full items-center justify-center gap-1.5 rounded-full bg-[#c64b32] py-2.5 text-xs font-semibold text-white">Mở hồ sơ đầy đủ <ExternalLink size={13} /></Link>
              </div> : null}
            </article>
          );
        })}
        {scopedCustomers.length === 0 ? <div className="rounded-2xl border border-dashed border-[#d2ad5d] bg-white py-10 text-center text-xs text-[#826f66]">Không tìm thấy khách hàng phù hợp.</div> : null}
      </section>
    </div>
  );
}
