  ...
  return (
    <section className="mt-4 rounded-2xl bg-[#fff8f3] p-4 ring-1 ring-[#eadbd1]">
      <div className="flex items-start gap-3">
        <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-white text-[#9f1d20] ring-1 ring-[#eadbd1]">
      ...
      {step === "request" ? (
        <form onSubmit={requestCode} className="mt-3 space-y-3">
          <label className="block text-xs font-semibold">Số điện thoại tài khoản
            <input required inputMode="tel" value={phone} onChange={(event) => setPhone(event.target.value)} className="mt-1.5 w-full rounded-xl border border-[#eadbd1] bg-white px-3 py-3 text-sm outline-none focus:border-[#9f1d20]" />
          </label>
          {message ? <p role="alert" className="text-[11px] leading-5 text-red-700">{message}</p> : null}
          {!deliveryConfigured ? <a href="/lien-he" className="inline-flex text-xs font-semibold text-[#9f1d20] underline underline-offset-2">Xem hotline các cơ sở</a> : null}
        ...
        <form onSubmit={confirmReset} className="mt-3 space-y-3">
          ...
          <label className="block text-xs font-semibold">Mã xác nhận 8 số
            <input required inputMode="numeric" pattern="[0-9]{8}" maxLength={8} value={code} onChange={(event) => setCode(event.target.value.replace(/\D/g, ""))} className="mt-1.5 w-full rounded-xl border border-[#eadbd1] bg-white px-3 py-3 text-center font-mono text-lg tracking-[0.3em] outline-none focus:border-[#9f1d20]" />
          </label>
          <label className="block text-xs font-semibold">Mật khẩu mới · tối thiểu 15 ký tự
            <input required type="password" minLength={15} maxLength={72} autoComplete="new-password" value={newPassword} onChange={(event) => setNewPassword(event.target.value)} className="mt-1.5 w-full rounded-xl border border-[#eadbd1] bg-white px-3 py-3 text-sm outline-none focus:border-[#9f1d20]" />
          </label>
          <label className="block text-xs font-semibold">Nhập lại mật khẩu mới
            <input required type="password" minLength={15} maxLength={72} autoComplete="new-password" value={confirmation} onChange={(event) => setConfirmation(event.target.value)} className="mt-1.5 w-full rounded-xl border border-[#eadbd1] bg-white px-3 py-3 text-sm outline-none focus:border-[#9f1d20]" />
          </label>
          <button disabled={submitting || !deliveryConfigured} className="flex w-full items-center justify-center gap-2 rounded-full bg-[#9f1d20] px-4 py-2.5 text-xs font-semibold text-white disabled:opacity-50">
