...
export function AdminQrManagement({ items, role }: { items: QrItem[]; role: "OWNER" | "BRANCH_MANAGER" }) {
  ...
  return <main className="mx-auto max-w-7xl px-3 py-3 sm:px-6 sm:py-5 lg:px-10">
    <header className="overflow-hidden rounded-2xl bg-gradient-to-r from-[#0b4f3c] via-[#17634a] to-[#22845a] p-4 text-center text-white shadow-xl">
    ...
    <section className="mt-3 rounded-2xl border border-[#d2ad5d]/55 bg-white p-2.5 shadow-sm">
      <div className="grid grid-cols-3 gap-1 rounded-xl bg-[#f6f0ec] p-1">{TABS.map((item) => { const Icon = item.icon; const active = tab === item.value; const count = items.filter((qr) => qr.targetType === item.value).length; return <button key={item.value} type="button" onClick={() => setTab(item.value)} className={cn("flex items-center justify-center gap-1.5 rounded-lg px-2 py-2 text-[10px] font-semibold", active ? "bg-[#c64b32] text-white shadow-sm" : "text-[#68574f]")}><Icon size={13} />{item.label}<span className={cn("rounded-full px-1.5 text-[8px]", active ? "bg-white/15" : "bg-white")}>{count}</span></button>; })}</div>
      <label className="mt-2 flex items-center gap-2 rounded-xl border border-[#e7d6ca] px-3 py-2"><Search size={14} className="text-[#826f66]" /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Tìm cơ sở, KTV hoặc doanh nghiệp..." className="min-w-0 flex-1 bg-transparent text-xs outline-none" /></label>
    </section>

    {message ? <p className={cn("mt-3 rounded-xl p-3 text-center text-xs", message.startsWith("Đã") ? "bg-[#eff7f3] text-[#0b6248]" : "bg-red-50 text-red-700")}>{message}</p> : null}
