  ...
  return (
    <>
      <section className="relative overflow-hidden rounded-2xl border border-[#d8b46a]/60 bg-white shadow-[0_12px_32px_rgba(74,44,28,0.08)]">
        ...
        <div className="grid grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-2 border-b border-[#eadbd1] bg-[#fffdfb] p-2.5 sm:px-4">
          <Link href={`/therapist?month=${previousMonth}`} className="flex h-9 w-9 items-center justify-center rounded-xl border border-[#eadbd1] bg-white text-[#7a2318]" aria-label="Tháng trước"><ChevronLeft size={17} /></Link>
          <div className="text-center text-sm font-semibold capitalize text-[#291714]">{monthLabel}</div>
          <Link href={`/therapist?month=${nextMonth}`} className="flex h-9 w-9 items-center justify-center rounded-xl border border-[#eadbd1] bg-white text-[#7a2318]" aria-label="Tháng sau"><ChevronRight size={17} /></Link>
        </div>
        ...
        <div className="relative bg-gradient-to-b from-[#fffaf4] to-white px-2 pb-3 pt-5 sm:px-4 sm:pb-4">
          ...
          <div className="mt-2.5 flex items-center justify-center gap-4 text-[8px] text-[#756861]"><span className="flex items-center gap-1"><i className="h-2 w-2 rounded-full bg-[#29a064]" /> Lịch tại cơ sở</span><span className="flex items-center gap-1"><i className="h-2 w-2 rounded-full bg-[#4b8ee8]" /> Đoàn Business</span></div>
        </div>
      </section>
      ...
      <section className="mt-3 overflow-hidden rounded-2xl border border-[#d8b46a]/60 bg-white shadow-sm">
        ...
        <div className="space-y-3 p-3 sm:p-4">
          ...
          {!selectedRegular.length && !selectedBusiness.length ? <p className="rounded-xl border border-dashed border-[#d9c8bc] p-7 text-center text-xs text-[#8a7a72]">Ngày này chưa có lịch được phân công.</p> : null}
        </div>
      </section>
    </>
  );
