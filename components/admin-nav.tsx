...
export function AdminNav() {
  ...
  if (session.role === "INVESTOR") {
    ...
    const investorNavItems = [
      { view: "overview" as const, href: "/admin#overview", label: "Tổng quan", icon: LayoutDashboard, activeIconClass: "bg-[#e8c665] text-[#2b1b13] shadow-[#e8c665]/25", activeTextClass: "text-[#f0d478]" },
    ...
    return (
      <>
        <header className="sticky top-0 z-40 border-b border-[#e8c665]/15 bg-[#160f0e]/95 text-white shadow-xl shadow-black/15 backdrop-blur">
          <div className="mx-auto flex max-w-7xl items-center justify-between gap-3 px-4 py-2.5 sm:px-6 lg:px-10">
            <button type="button" onClick={() => selectInvestorView("overview")} className="flex min-w-0 items-center gap-2.5 text-left">
              <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full border border-[#e8c665]/30 bg-[#e8c665]/10 text-[#e8c665]"><Landmark size={17} /></span>
              <span className="min-w-0"><span className="block truncate text-sm font-semibold">Tâm An Investor</span><span className="block truncate text-[9px] text-[#e8c665]/75">Báo cáo đầu tư · chỉ xem</span></span>
            </button>
        ...
        <nav className="fixed inset-x-0 bottom-0 z-40 border-t border-[#e8c665]/15 bg-[#160f0e]/96 pb-[env(safe-area-inset-bottom)] text-white backdrop-blur md:hidden">
        ...
        {notificationsOpen ? (
          <div className="fixed inset-0 z-50">
            ...
            <section className="absolute inset-x-0 bottom-0 flex h-[88dvh] flex-col overflow-hidden rounded-t-[2rem] border-t border-[#e8c665]/18 bg-[#1b1211] text-white shadow-2xl sm:bottom-auto sm:left-auto sm:right-6 sm:top-16 sm:h-auto sm:max-h-[86dvh] sm:w-[430px] sm:rounded-2xl sm:border">
              <div className="shrink-0 bg-gradient-to-br from-[#3c271f] via-[#271817] to-[#160f0e] p-4">
                <div className="flex items-start justify-between gap-3">
                  <div><p className="flex items-center gap-1.5 text-sm font-semibold"><Bell size={16} className="text-[#f5d982]" /> Bản tin Nhà đầu tư</p><p className="mt-1 text-[10px] text-white/55">Thông tin từ Admin, đội ngũ đầu tư và Investor Care.</p></div>
                ...
                <div className="mt-3 flex items-center justify-between rounded-xl border border-white/7 bg-white/[0.045] px-3 py-2">
                  <span className="text-[10px] text-white/60"><strong className="text-sm text-[#f5d982]">{unreadInvestorNotifications}</strong> bản tin chưa đọc</span>
                  <button type="button" onClick={markAllInvestorNotificationsRead} disabled={unreadInvestorNotifications === 0} className="inline-flex items-center gap-1 rounded-full bg-[#e8c665] px-2.5 py-1.5 text-[10px] font-bold text-[#3a261a] disabled:opacity-40"><CheckCheck size={13} /> Đọc tất cả</button>
                </div>
              </div>
              ...
              <div className="min-h-0 flex-1 space-y-2 overflow-y-auto p-3 pb-[calc(env(safe-area-inset-bottom)+0.75rem)] sm:pb-3">
                ...
                {!investorNotificationsLoading && filteredInvestorNotifications.map((item) => {
                  ...
                  return <div key={item.id} className={cn("flex items-start gap-2 rounded-2xl border p-2.5", item.read ? "border-white/7 bg-white/[0.025]" : "border-[#e8c665]/22 bg-gradient-to-r from-[#e8c665]/[0.09] to-white/[0.025]") }>
                    <button type="button" onClick={() => openInvestorNotification(item)} className="flex min-w-0 flex-1 items-start gap-2.5 text-left">
                      <span className={cn("mt-0.5 flex h-9 w-9 shrink-0 items-center justify-center rounded-xl", item.type === "FINANCE" ? "bg-emerald-300/10 text-emerald-200" : item.type === "PROMOTION" ? "bg-[#e8c665]/10 text-[#f5d982]" : "bg-sky-300/10 text-sky-200")}><Icon size={16} /></span>
                      <span className="min-w-0 flex-1"><span className="flex items-start gap-1"><strong className="min-w-0 flex-1 text-[11px] leading-4">{item.title}</strong>{!item.read ? <i className="mt-1 h-2 w-2 shrink-0 rounded-full bg-[#e8c665]" /> : null}</span><span className="mt-1 block text-[10px] leading-4 text-white/48">{item.body}</span><span className="mt-1.5 block text-[9px] font-medium text-white/30">{source} · {notificationTime(item.createdAt)}</span></span>
                      <ChevronRight size={14} className="mt-3 shrink-0 text-[#e8c665]/65" />
                    </button>
  ...
  return (
    <>
            ...
            <div className="relative hidden sm:block">
              <button type="button" onClick={() => setRoleOpen((value) => !value)} className="flex items-center gap-2 rounded-full border border-[#eadbd1] bg-[#fffaf6] py-1.5 pl-2 pr-3 text-left">
                ...
                <span>
                  ...
                  <span className="block text-[9px] text-[#8a7a72]">{session.title}</span>
                </span>
                <ChevronDown size={13} className={cn("text-[#8a7a72] transition", roleOpen && "rotate-180")} />
              </button>
              {roleOpen ? (
                <div className="absolute right-0 top-12 w-64 rounded-2xl border border-[#eadbd1] bg-white p-2 shadow-xl">
                  <div className="rounded-xl bg-[#fff7ec] px-3 py-2">
                    <p className="text-xs font-semibold text-[#5c3a1e]">Phạm vi: {session.branchLabel}</p>
                    <p className="mt-0.5 text-[10px] leading-4 text-[#8a7a72]">{session.permissions.length} nhóm quyền đang được cấp</p>
                  </div>
                  <Link href="/dang-nhap-quan-tri" className="mt-1.5 flex w-full items-center gap-2 rounded-lg px-3 py-2 text-xs font-medium hover:bg-[#fff7f3]">
      ...
      <nav className="fixed inset-x-0 bottom-0 z-40 border-t border-[#eadbd1] bg-white/95 backdrop-blur pb-[env(safe-area-inset-bottom)] md:hidden">
        <div className="mx-auto flex max-w-lg items-stretch justify-around">
        ...
        <div className="fixed inset-0 z-50 overflow-y-auto bg-[#fffaf6] text-[#191414]">
          ...
          <main className="mx-auto max-w-3xl px-4 py-5 pb-[calc(env(safe-area-inset-bottom)+1.5rem)] sm:px-6">
            ...
            <div className="mt-3 overflow-hidden rounded-2xl border border-[#eadbd1] bg-white shadow-sm">
              ...
              {filteredAdminNotifications.length === 0 ? <div className="p-10 text-center text-xs text-[#8a7a72]">Không có thông báo phù hợp bộ lọc.</div> : null}
            </div>
          </main>
        </div>
      ) : null}
      ...
      {moreOpen ? (
        <div className="fixed inset-0 z-50 overflow-y-auto bg-[#fffaf6] md:hidden">
          <div className="min-h-dvh p-4 pb-[calc(env(safe-area-inset-bottom)+1rem)] pt-[calc(env(safe-area-inset-top)+1rem)]">
            <div className="relative mb-4 flex flex-col items-center rounded-2xl bg-gradient-to-br from-[#351b15] via-[#5c2718] to-[#8c211d] p-4 text-center text-white shadow-lg">
              <div className="flex flex-col items-center">
                <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-white/12 text-[#f5d982] ring-1 ring-white/20">{session.role === "OWNER" ? <ShieldCheck size={18} /> : <Building2 size={18} />}</span>
