      ...
      {moreOpen ? (
        <div className="fixed inset-0 z-50 overflow-y-auto bg-[#fdf8f3] md:hidden">
          <div className="min-h-dvh p-4 pb-[calc(env(safe-area-inset-bottom)+1rem)] pt-[calc(env(safe-area-inset-top)+1rem)]">
            ...
            {canManageFinance ? <Link href="/admin/qr-management" onClick={() => setMoreOpen(false)} className="mt-4 flex items-center gap-2.5 rounded-xl border border-[#e8d2c4] bg-[#fbf2e7] p-3"><span className="flex h-9 w-9 items-center justify-center rounded-full bg-[#76551d] text-white"><QrCode size={16} /></span><span><span className="block text-xs font-semibold">QR khách trực tiếp & Business</span><span className="block text-[10px] text-[#607a6b]">Tải mã dùng tại cơ sở hoặc doanh nghiệp</span></span></Link> : null}
