  ...
  return (
    <div>
      <div className="mt-3 overflow-hidden rounded-2xl border border-[#e7d4ca] bg-white shadow-sm">
        <div className="flex items-center justify-between gap-2 border-b border-dashed border-[#eadbd1] bg-[#fffaf6] px-3 py-2 text-[10px] font-semibold">
          <span className="inline-flex items-center gap-1 text-[#1d6c40]"><ShieldCheck size={12} /> VietQR đã điền sẵn</span>
        ...
        <div className="p-4 text-center">
          <div className="mx-auto w-fit rounded-2xl border border-[#eadbd1] bg-white p-2.5 shadow-md shadow-[#5c3a1e]/10">
          ...
          <div className="mt-3 grid grid-cols-2 gap-2">
            ...
            <button type="button" onClick={openBankPicker} className="inline-flex min-h-11 items-center justify-center gap-1.5 rounded-full bg-gradient-to-b from-[#c22630] to-[#8f151a] px-3 text-xs font-semibold text-white shadow-md shadow-black/15">
              <Smartphone size={15} /> Mở app ngân hàng
            </button>
          </div>
          {saveNotice ? <p role="status" className="mt-2 rounded-xl bg-[#fff7ec] px-3 py-2 text-[10px] leading-4 text-[#805914]">{saveNotice}</p> : null}
        </div>
      </div>
        ...
        <div className="fixed inset-0 z-[90] bg-black/45 backdrop-blur-[2px]" role="presentation" onClick={() => setShowBankApps(false)}>
          <section role="dialog" aria-modal="true" aria-label="Chọn ứng dụng ngân hàng" onClick={(event) => event.stopPropagation()} className="absolute inset-x-0 bottom-0 mx-auto flex max-h-[82dvh] w-full max-w-xl flex-col overflow-hidden rounded-t-[1.75rem] bg-[#fffaf6] shadow-2xl">
            ...
            <div className="min-h-0 flex-1 overflow-y-auto px-3 py-3 pb-[max(1rem,env(safe-area-inset-bottom))]">
              {loadingApps ? <p className="mb-2 flex items-center justify-center gap-2 py-2 text-xs text-[#8a7a72]"><Loader2 size={14} className="animate-spin" /> Đang cập nhật danh sách ngân hàng Việt Nam...</p> : null}
              ...
              {!visibleApps.length ? <p className="rounded-2xl border border-dashed border-[#d9c9c0] bg-white p-5 text-center text-xs text-[#8a7a72]">Không tìm thấy ngân hàng phù hợp. Bạn vẫn có thể lưu QR và quét trong bất kỳ ứng dụng ngân hàng nào.</p> : null}
